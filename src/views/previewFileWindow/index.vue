  ...
  <div class="size-full bg-[--right-bg-color]">
    ...
    <n-scrollbar
      ...
      <div class="flex flex-col gap-4 bg-#808080">
        ...
        <div v-else class="text-gray-500">📄 미리볼 수 있는 문서가 없습니다</div>
      </div>
    </n-scrollbar>
  </div>
</template>
...
const uiData = reactive({
  ...
  file: new File([], ''), // 로컬 파일이 발견된 경우에만 사용
...
const updateFile = async (absolutePath: string, exists: boolean) => {
  try {
    if (exists) {
      uiData.fileLoading = false // 초기값을 false로 설정하여 상태를 깨끗하게 유지
      ...
      uiData.fileLoading = true // 파일 로드 완료, 렌더링 준비 완료
      console.log('로컬 파일 업데이트됨 ', file.file.size, uiData.file.size)
    } else {
    ...
    uiData.fileLoading = false // 읽기 실패 시에도 false로 표시
  }
}
