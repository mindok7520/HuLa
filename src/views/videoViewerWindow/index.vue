...
const canGoPrevious = computed(() => {
  ...
  // 로컬 비디오: 항상 true 반환, 실제 판단은 클릭 시 수행
  return true
})
...
// 현재 비디오가 위치한 폴더의 모든 비디오 파일 가져오기
const getVideosFromCurrentFolder = async (currentVideoPath: string) => {
  try {
    ...
    return videoFiles.sort() // 파일명 순으로 정렬
  } catch (error) {
...
const onVideoLoaded = () => {
  if (videoRef.value) {
    ...
    videoRef.value
      ...
      .catch((error) => {
        console.warn('자동 재생 실패, 사용자 상호작용이 필요할 수 있음:', error)
        isPlaying.value = false
      })
  }
}
