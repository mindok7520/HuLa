...
const { openMsgSession } = useCommon()
// 라우트 매개변수 또는 공유 상태에서 검색 쿼리 가져오기
const searchQuery = ref('')
// 검색 결과
const searchResults = ref<SessionItem[]>([])
// 기록 - localStorage 사용하여 저장
...
const handleSearch = (value: string) => {
  ...
  // 검색 키워드가 있는 경우, 여기서 키워드 기록에 저장할 수 있음 (현재 구현은 아직 검색 키워드를 저장하지 않음)
  if (value) {
    saveToHistory(value)
  }
}
...
const loadHistory = () => {
  try {
    ...
    if (history) {
      ...
      // 타임스탬프 내림차순 정렬, 최근 방문한 항목을 앞에 배치
