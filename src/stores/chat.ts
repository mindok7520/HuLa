...
// 단일 세션의 메모리 내 메시지 보존 제한, 백그라운드 세션의 무한 증가 방지
...
export const useChatStore = defineStore(
  ...
  () => {
    ...
    // 세션 목록 빠른 조회 Map, roomId를 통한 O(1) 조회
    const sessionMap = ref<Record<string, SessionItem>>({})
    // 세션 목록 로드 상태
    const sessionOptions = reactive({ isLast: false, isLoading: false, cursor: '' })
    // 메시지 동기화 로드 상태 (동기화 중 팁 표시에 사용)
    const syncLoading = ref(false)

    // 영구 저장된 읽지 않은 수를 메모리의 세션 객체로 동기화하여, 새로고침이나 계정 전환 후에도 이전 읽지 않은 상태를 볼 수 있도록 함
    ...
    // 세션 삭제 또는 데이터 정리 시 캐시도 함께 제거하여 이전 데이터 오염 방지
    ...
    // 기존 세션 목록을 sessionMap에 동기화, 영구 저장 복구 또는 요청 실패 시 map이 비어 있는 문제 해결
    ...
    // 세션 가져오기 대체 수단, map 우선 사용, 없을 시 목록에서 찾아 map에 채움
    ...
    // 최근에 세션을 읽음으로 표시했을 때의 활성 시간 기록, 다음 로그인 시 "오래된 읽지 않음" 식별에 사용
    const lastReadActiveTime = ref<Record<string, number>>({})

    // 세션 목록 새로고침 후 서버에서 반환된 "오래된 읽지 않음" 처리 — 로컬에서 이미 읽음(읽지 않은 수 0)이고 활성 시간이 변하지 않았지만 서버가 여전히 읽지 않음으로 반환하는 경우
    const reconcileStaleUnread = async (prevSessions?: Record<string, SessionItem>) => {
      ...
      for (const session of sessionList.value) {
        ...
        // "로컬 이전이 0, 현재 양수, 유효한 활성 시간이 존재하며 변경되지 않음"인 경우에만 오래된 읽지 않음으로 간주
        if (prevUnread === 0 && currentUnread > 0 && hasValidActiveTime && session.activeTime === prev.activeTime) {
          console.log('[Chat][reconcileStaleUnread] 오래된 읽지 않음 지우기', session.roomId, {
            prevUnread,
            currentUnread,
            activeTime: session.activeTime
          })
          updateSession(session.roomId, { unreadCount: 0 })
          // 다음 새로고침 시 다시 나타나는 것을 방지하기 위해 읽음 보고 보상
          promises.push(
            markMsgRead(session.roomId).catch((error) => {
              console.error('[chat] 읽음 보고 보상 실패:', error)
            })
          )
        }
      }
    ...
    // 로컬에 기록된 "마지막 읽음 활성 시간"을 사용하여 오래된 읽지 않은 항목 정리, 재로그인 시 잠깐 깜박이는 현상 방지
    const reconcileUnreadWithReadHistory = async () => {
      ...
      for (const session of sessionList.value) {
        ...
        if (currentUnread > 0 && lastReadTime > 0 && (activeTime === 0 || activeTime <= lastReadTime)) {
          console.log('[Chat][reconcileUnreadWithReadHistory] 마지막 읽음에 의해 지우기', session.roomId, {
            activeTime,
            lastReadTime,
            currentUnread
          })
    ...
    const currentSessionInfo = computed(() => {
      ...
      // sessionMap에서 직접 조회 (페이지 새로고침 후 자동 복구)
     ...
     * 채팅방 전환
     * @description
     * 사용자가 다른 채팅방으로 전환할 때 호출되어 전체 채팅방 전환 프로세스를 수행합니다.
     * 이 메서드는 이전 방의 메시지 데이터를 지우고 새 방의 메시지를 다시 로드하며 관련 상태 초기화를 처리합니다.
     */
    const changeRoom = async () => {
      ...
      // currentSession이 존재하지 않으면 바로 반환
    ...
    const chatMessageList = computed(() => {
      ...
      // Rust 백엔드에서 계산된 timeBlock을 직접 사용, 프론트엔드 계산 안 함
    ...
    /**
     * 로그인 후 모든 세션의 메시지를 한 번 로드
     * @description
     * 제어된 동시성 로드(p-limit)를 사용하여 많은 세션이 있을 때 UI 차단 방지
     * - 최근 활성 세션 우선 로드
     * - 동시성을 5로 제한하여 성능과 서버 부하 균형 유지
     * - Promise.allSettled를 사용하여 일부 실패가 다른 세션에 영향을 주지 않도록 보장
     */
    const setAllSessionMsgList = async (size = pageSize) => {
      await info('모든 세션 메시지 목록 초기 설정')

      if (sessionList.value.length === 0) return

      // 활성 시간순 정렬, 최근 세션 우선 로드
      const sortedSessions = [...sessionList.value].sort((a, b) => b.activeTime - a.activeTime)

      // 동시성 제한 생성 (최대 5개 요청 동시 실행)
      ...
      await info(`세션 메시지 로드 완료: 성공 ${successCount}/${sortedSessions.length}, 실패 ${failCount}`)

      // 실패한 세션 기록 (선택 사항)
      if (failCount > 0) {
        results.forEach((result, index) => {
          if (result.status === 'rejected') {
            console.warn(`세션 ${sortedSessions[index].roomId} 메시지 로드 실패:`, result.reason)
          }
        })
      }
    }
    ...
    const getSessionList = async (_isFresh = false) => {
      try {
        ...
        // 이전 세션의 오래된 읽지 않음 표시 방지, 동기화 중에는 메시지 읽지 않음 수를 0으로 설정하고 가져오기가 완료된 후 다시 계산
        ...
        const data: any = await invokeWithErrorHandler(TauriCommand.LIST_CONTACTS, undefined, {
          customErrorMessage: '세션 목록 가져오기 실패',
          errorType: ErrorType.Network
        }).catch(() => {
        ...
        // 오래된 읽지 않음 보상 후 한 번에 읽지 않은 배지 업데이트, 로드 과정 중 깜박임 방지
        await reconcileStaleUnread(prevSessions)
        await reconcileUnreadWithReadHistory()
        await clearCurrentSessionUnread()
        updateTotalUnreadCount()
        // 현재 세션이 서버에서 여전히 읽지 않음으로 표시된 경우, 능동적으로 보고하고 0으로 초기화하여 버블이 멈추는 것 방지
        ...
        if (currentRoomId) {
          ...
          if (currentSession?.unreadCount) {
            try {
              ...
              console.error('[chat] 세션 목록 동기화 후 읽음 보고 실패:', error)
            }
            markSessionRead(currentRoomId)
            // 현재 세션 읽지 않음 제거 후 총 읽지 않은 수를 다시 계산하여 Dock 아이콘이 올바르게 업데이트되도록 함
            updateTotalUnreadCount()
          }
        }
        globalStore.unreadReady = true
        ...
        console.error('세션 목록 가져오기 실패11:', e)
        sessionOptions.isLoading = false
        // 오류 발생 시에도 읽지 않음 표시 복구, 배지가 오랫동안 숨겨지는 것 방지
        globalStore.unreadReady = true
        unreadCountManager.refreshBadge(globalStore.unReadMark, feedStore.unreadCount)
      } finally {
        sessionOptions.isLoading = false
    ...
    const updateSession = (roomId: string, data: Partial<SessionItem>) => {
      ...
      if (session) {
        ...
        if (index !== -1) {
          ...
          console.warn('[updateSession] 세션이 sessionList에 없음:', roomId)
        }
        ...
        if ('unreadCount' in data && typeof updatedSession.unreadCount === 'number') {
          console.log('[updateSession] 읽지 않은 수 업데이트:', roomId, updatedSession.unreadCount)
          persistUnreadCount(roomId, updatedSession.unreadCount)
          requestUnreadCountUpdate(roomId)
        }

        // 방해 금지 상태가 업데이트된 경우 전역 읽지 않은 수를 다시 계산해야 함
        if ('muteNotification' in data) {
          requestUnreadCountUpdate()
        }
      } else {
        console.warn('[updateSession] 세션이 존재하지 않음:', roomId)
      }
    }

    // 세션 마지막 활성 시간 업데이트, 업데이트 과정 중 세션이 존재하지 않으면 세션을 새로고침
    ...
    const getSession = (roomId: string) => {
      ...
      // O(1) 조회 (페이지 새로고침 후 영구 저장에서 자동 복구)
      return resolveSessionByRoomId(roomId)
    }
    ...
    const pushMsg = async (msg: MessageType, options: { isActiveChatView?: boolean; activeRoomId?: string } = {}) => {
      ...
      if (session) {
        ...
        // 읽지 않은 수 업데이트: 자신이 보낸 메시지가 아니고, 현재 활성 채팅 뷰에 없거나 다른 방의 메시지일 때만 증가
        ...
        if (shouldIncreaseUnread) {
          ...
          console.log('[pushMsg] 읽지 않은 수 증가:', msg.message.roomId, updateData.unreadCount, {
            isActiveChatView,
            targetRoomId,
        ...
        // updateSession을 사용하여 통합 업데이트, 반응형 트리거 보장
        updateSession(msg.message.roomId, updateData)
      } else {
        // 세션이 존재하지 않음, 새 세션 추가
        await addSession(msg.message.roomId)
        // 새 세션 추가 후, 자신이 보낸 메시지가 아니고 현재 활성 뷰가 아니면 읽지 않은 수 설정
        const isSelfMessage = msg.fromUser.uid === userStore.userInfo!.uid
        const shouldIncreaseUnread = !isSelfMessage && (!isActiveChatView || msg.message.roomId !== targetRoomId)
        if (shouldIncreaseUnread) {
          updateSession(msg.message.roomId, { unreadCount: 1 })
          console.log('[pushMsg] 새 세션 읽지 않은 수 증가:', msg.message.roomId)
        }
      }

      // 수신된 메시지에 자신을 멘션한 내용이 포함되어 있으면 시스템 알림 전송
      ...
      // 백그라운드 세션에 메시지가 장기간 누적되어 제한을 초과하는 것을 방지 (현재 읽고 있는 세션은 온전하게 유지)
    ...
    const recordRecallMsg = (data: {
      ...
      // 원래 originalType과 originalContent를 우선 사용하여 경쟁 상태로 인한 유형 변경 방지
    ...
    const updateRecallMsg = async (data: RevokedMsgType) => {
      ...
      if (message && typeof data.recallUid === 'string') {
        ...
        if (isRecallerCurrentUser) {
          // 현재 사용자가 회수 작업을 수행한 경우
          if (data.recallUid === senderUid) {
            // 자신의 관점
            recallMessageBody = '메시지를 회수했습니다'
          } else {
            // 타인의 메시지 회수
            recallMessageBody = `${senderName}님의 메시지를 회수했습니다`
          }
        } else {
          // 현재 사용자가 회수 작업을 수행하지 않은 경우
          if (isGroup) {
            // 그룹 채팅에서 회수자 닉네임 표시
            const recallerLabel = recallerName || '상대방'
            if (isSenderCurrentUser) {
              recallMessageBody = `${recallerLabel}님이 회원님의 메시지를 회수했습니다`
            } else {
              recallMessageBody = `${recallerLabel}님이 메시지를 회수했습니다`
            }
          } else {
            // 비그룹 채팅은 기존 1:1 채팅 로직 유지
            if (isSenderCurrentUser) {
              // 현재 사용자가 회수된 메시지의 발신자인 경우 (피회수자 관점)
              recallMessageBody = '상대방이 회원님의 메시지를 회수했습니다'
            } else {
              // 현재 사용자가 제3자인 경우 (다른 멤버 관점)
              recallMessageBody = '상대방이 메시지를 회수했습니다'
            }
          }
        }
        ...
        try {
          await invokeWithErrorHandler(
            ...
            {
              customErrorMessage: '회수 메시지 상태 업데이트 실패',
              errorType: ErrorType.Client
            }
          )
    ...
    const markSessionRead = (roomId: string) => {
      const session = resolveSessionByRoomId(roomId)
      if (!session) {
        console.log('[markSessionRead] 세션이 존재하지 않음:', roomId)
        return
      }
      if (session.unreadCount === 0) {
        console.log('[markSessionRead] 읽지 않은 수가 이미 0임, 건너뜀:', roomId)
        requestUnreadCountUpdate(roomId)
        return
      }

      console.log('[markSessionRead] 읽지 않은 수 지우기:', roomId, '현재 읽지 않은 수:', session.unreadCount)
      ...
      // 전역 읽지 않은 수 즉시 새로고침, 디바운스 대기 방지
      updateTotalUnreadCount()
    }

    // 현재 세션의 읽지 않음 정리 (재연결/재로그인 후에도 해당 세션에 머물러 있을 때의 보조 수단)
    const clearCurrentSessionUnread = async () => {
      ...
      try {
        ...
        console.error('[chat] 읽음 보상 보고 실패:', error)
      }
    ...
    timerWorker.onmessage = (e) => {
      ...
      if (type === 'timeout') {
        ...
        // 모든 타이머가 완료되면 안전하게 리소스 정리를 할 수 있습니다.
    ...
    const clearAllExpirationTimers = () => {
      ...
      // recalledMessages도 함께 정리하여 메모리 누적 방지
    ...
    const clearRedundantMessages = (roomId: string, limit: number = pageSize) => {
      ...
      // 메시지를 배열로 변환하고 메시지 ID 역순으로 정렬 (최신 메시자가 앞쪽)
      ...
      // 커서를 현재 메모리에서 가장 오래된 메시지 ID로 업데이트하여, 이후 '더 불러오기' 시 데이터베이스에서 이전 메시지를 불러올 수 있도록 보장
      if (fallbackCursor) {
        messageOptions[roomId] = {
          ...messageOptions[roomId],
      ...
      // 메모리 압축 트리거 지점을 쉽게 파악할 수 있도록 콘솔에 정리 정보 출력
      console.info(
        '[chat][trim]',
        `roomId=${roomId}`,
    ...
    /**
     * 현재 채팅방 메시지 목록 초기화 및 새로고침
     * @description
     * 현재 채팅방의 모든 로컬 메시지 캐시를 지우고 서버에서 최신 메시지 목록을 다시 가져옵니다.
     * 메시지 강제 새로고침이 필요한 시나리오(예: 최신 서버 데이터 표시 보장)에서 주로 사용됩니다.
     */
    const resetAndRefreshCurrentRoomMessages = async () => {
      ...
      try {
        // 1. 메시지 데이터 비우기 (경쟁 상태 방지)
        if (messageMap[requestRoomId]) {
          messageMap[requestRoomId] = {}
        }

        // 2. 메시지 로드 상태 초기화, 최신 메시지 가져오기를 위해 커서 강제 초기화
        ...
        // 4. getPageMsg를 직접 호출하여 최신 메시지 가져오기 (빈 커서 강제 사용)
        await getPageMsg(pageSize, requestRoomId, '')

        console.log('[Network] 현재 채팅방 메시지 목록 초기화 및 새로고침 완료')
      } catch (error) {
        console.error('[Network] 메시지 목록 초기화 및 새로고침 실패:', error)
        // 실패 시 로드 상태 초기화 보장
