...
export const useGroupStore = defineStore(
  ...
  () => {
    ...
    const userListOptions = reactive({ isLast: false, loading: true, cursor: '' }) // 페이지 로드 관련 상태
    ...
    const updateOnlineNum = (options: { uid?: string; roomId?: string; onlineNum?: number; isAdd?: boolean }) => {
      ...
      if (roomId) {
        // roomId가 전달되면 해당 방의 onlineNum만 수정
        ...
        // roomId가 전달되지 않으면 전역 업데이트
        const roomIds = getRoomIdsByUid(uid!)

        // 찾은 모든 방 ID를 반복하여 해당 그룹의 온라인 인원 업데이트
    ...
    const getGroupUserList = async (roomId: string, forceRefresh = false) => {
      if (!roomId) {
        console.warn('[group] 멤버 새로고침 건너뜀, 유효하지 않은 방 ID:', roomId)
        return []
      }
     ...
     * @param roomId 그룹 채팅 방 ID, 선택 사항, 기본값은 현재 방; 'all'을 전달하면 모든 방에서 해당 사용자 업데이트
     * @returns 업데이트 성공 여부
     */
    const updateUserItem = (uid: string, updates: Partial<UserItem>, roomId: string | 'all' = 'all'): boolean => {
      if (!uid || typeof uid !== 'string') {
        console.warn('[updateUserItem] 유효하지 않은 uid:', uid)
        return false
      }

      if (!updates || typeof updates !== 'object') {
        console.warn('[updateUserItem] 유효하지 않은 업데이트 페이로드:', updates)
        return false
      }
      ...
      refreshTargets.forEach((validRoomId) => {
        getGroupUserList(validRoomId, true).catch((error) => {
          console.error('[group] 멤버 새로고침 실패:', error)
        })
      })
    ...
    const addUserItem = (userItem: UserItem, roomId?: string): boolean => {
      if (!userItem || typeof userItem !== 'object' || !userItem.uid) {
        console.warn('[addUserItem] 유효하지 않은 사용자 정보:', userItem)
        return false
      }

      const targetRoomId = roomId || globalStore.currentSessionRoomId
      if (!targetRoomId) {
        console.warn('[addUserItem] 대상 방 ID를 결정할 수 없습니다')
        return false
      }
    ...
    const removeUserItem = (uid: string, roomId?: string): boolean => {
      if (!uid || typeof uid !== 'string') {
        console.warn('[removeUserItem] 유효하지 않은 uid:', uid)
        return false
      }

      const targetRoomId = roomId || globalStore.currentSessionRoomId
      if (!targetRoomId) {
        console.warn('[removeUserItem] 대상 방 ID를 결정할 수 없습니다')
        return false
      }
    ...
    const exitGroup = async (roomId: string) => {
      ...
      // 세션 목록 업데이트, 전달된 roomId 사용
      chatStore.removeSession(roomId)

      // 나가는 방이 현재 선택된 세션인 경우, 새로운 현재 세션으로 전환
    ...
    /**
     * 온라인 상태 변경 시 그룹 멤버 목록 새로고침 처리
     */
    const refreshGroupMembers = async () => {
      // 채널 멤버 목록 항상 새로고침
      await getGroupUserList('1', true)

      // 현재 선택된 것이 그룹 채팅이고 채널이 아닌 경우, 현재 그룹 채팅의 멤버 목록도 새로고침
    ...
    /**
     * 그룹 데이터 재설정
     * 세션 전환 시 현재 그룹의 데이터 지우기에 사용됨
     * @param roomId 선택 사항, 정리할 방 ID 지정, 지정하지 않으면 모두 정리
     */
    ...
    /**
     * 사용자 ID로 해당 사용자가 있는 모든 방 ID 찾기
     * @param uid 사용자 ID
     * @returns 해당 사용자가 포함된 모든 방 ID 배열
     */
