/**
 * 전역 열거형 파일
 * 열거형 값이 전역적으로 사용되어야 하는 경우 이 파일에 정의하세요. 다른 열거형 값은 해당 파일에 정의하세요.
 * 정의 규칙:
 *  열거형 이름: XxxEnum
...
/**
 * AI 메시지 콘텐츠 유형 열거형
 * AI가 생성한 메시지 콘텐츠 유형(텍스트, 이미지, 비디오, 오디오) 식별에 사용
 */
...
export enum ScrollIntentEnum {
  ...
  LOAD_MORE = 'load_more' // 이전 메시지 더 불러오기
}
