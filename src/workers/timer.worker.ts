...
/** 문자열 및 숫자 유형의 키를 지원하도록 타입 정의 수정 */
...
const logDebugInfo = (msgId: TimerId, remainingTime: number) => {
  // 로그 기능이 활성화된 경우에만 디버그 메시지 전송 및 로그 출력
  ...
  // 주요 시점(마지막 5초 또는 10초마다)에서만 로그 출력
...
self.onmessage = (e) => {
  ...
  switch (type) {
    case 'startReconnectTimer': {
      // 메인 스레드에서 타이머 재시작 이벤트를 보내고, 지연 후 메인 스레드로 reconnectTimeout 이벤트 반환
    ...
    case 'startTimer': {
      activeTimers++
      safeLog(`[Worker] 타이머 시작: ${msgId}, 기간: ${duration}ms`)
      // 숫자 유형의 msgId 사용
      ...
      // 로그가 활성화된 경우에만 디버그 타이머를 생성하여 불필요한 성능 오버헤드 방지
