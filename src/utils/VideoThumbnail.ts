...
// 썸네일을 주어진 크기와 품질로 압축하며, 비율을 유지합니다. 압축된 File을 반환합니다.
const compressThumbnail = async (
  ...
  return new Promise((resolve, reject) => {
      ...
      canvas.toBlob(
        (blob) => {
          if (blob) {
            ...
            reject(new AppException('썸네일 압축 실패'))
          }
        },
        'image/jpeg',
        quality
    ...
    img.onerror = () => {
      reject(new AppException('썸네일 로드 실패'))
    }
...
// 브라우저 기능을 사용하여 비디오 첫 프레임을 캡처하여 썸네일 생성, Rust 지원이 없을 경우의 폴백 방안으로 사용
  ...
  try {
    const blob = await new Promise<Blob>((resolve, reject) => {
      ...
      video.onseeked = () => {
        ...
        const ctx = canvas.getContext('2d')
        if (!ctx) {
          reject(new AppException('비디오 썸네일 생성 실패: 캔버스 컨텍스트를 가져올 수 없음'))
          return
        }
        ...
        canvas.toBlob(
          (blobResult) => {
            if (blobResult) {
              ...
              reject(new AppException('비디오 썸네일 생성 실패: 캔버스 변환 실패'))
            }
          },
          'image/jpeg',
          0.8
      ...
      video.onerror = () => reject(new AppException('비디오 썸네일 생성 실패: 비디오를 로드할 수 없음'))
    })
...
// 비디오 썸네일 생성: 우선 Rust를 호출하여 첫 프레임을 가져오고, 실패하거나 모바일인 경우 프론트엔드 캡처로 폴백
