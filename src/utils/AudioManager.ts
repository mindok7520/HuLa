  ...
  async play(audio: HTMLAudioElement, audioId: string): Promise<void> {
    if (this.currentAudio) {
      if (this.currentAudioId !== audioId) {
        // 다른 오디오인 경우 현재 재생 중인 오디오 즉시 중지
        this.stop()
      } else {
        // 동일한 오디오인 경우 병렬 재생을 피하기 위해 먼저 리셋 필요
    ...
    try {
      ...
      // 네트워크 재연결 또는 빠른 전환 시 audio.play가 중단되어 AbortError가 발생할 수 있으므로 무시함
  ...
  pause(): void {
    if (this.currentAudio) {
      try {
        ...
        // 오디오 요소가 이미 소멸되었거나 상태가 비정상인 오류 무시
        console.warn('오디오 일시 중지 중 오류 발생:', error)
        this.currentAudio = null
        this.currentAudioId = null
      }
    }
  ...
  stop(): void {
    if (this.currentAudio) {
      try {
        ...
        // 오디오 요소가 이미 소멸되었거나 상태가 비정상인 오류 무시
        console.warn('오디오 중지 중 오류 발생:', error)
      } finally {
        this.currentAudio = null
        this.currentAudioId = null
  ...
  stopAll(): void {
    ...
    document.querySelectorAll('audio').forEach((audio) => {
      try {
        ...
        // 오디오 요소가 이미 소멸되었거나 상태가 비정상인 오류 무시
        console.warn('페이지 오디오 중지 중 오류 발생:', error)
      }
    })
