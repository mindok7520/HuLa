...
export const useContextMenu = (ContextMenuRef: Ref, isNull?: Ref<boolean>) => {
  ...
  const handleContextMenu = (e: MouseEvent) => {
    ...
    // 현재 우클릭 대상이 기존 텍스트 선택을 포함하는 경우, 복사/번역에 영향을 주지 않도록 사용자 선택 유지
    ...
    x.value = e.clientX
    y.value = e.clientY
    window.addEventListener('wheel', preventDefault, { passive: false }) // 휠 스크롤 방지
  }

  const closeMenu = (event: any) => {
    /** 클릭 대상이 .context-menu 클래스 요소가 아닐 때만 메뉴 닫기 */
    if (!event.target.matches('.context-menu, .context-menu *')) {
      handleVirtualListScroll(false)
      showMenu.value = false
      enableTextSelection() // 텍스트 선택 기능 복원
    }
    window.removeEventListener('wheel', preventDefault) // 휠 스크롤 방지 제거
  }
  ...
  onMounted(() => {
    ...
    // 여기서는 div의 우클릭만 감지합니다. 다른 요소의 우클릭을 감지하려면 해당 요소에서 감지해야 합니다.
    div.addEventListener('contextmenu', handleContextMenu)
    // window의 우클릭을 감지해야 합니다. 그렇지 않으면 우클릭이 div의 우클릭 이벤트를 트리거하여 메뉴가 닫히지 않고 기본 우클릭 메뉴를 차단합니다.
