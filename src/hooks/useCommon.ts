...
export const useCommon = () => {
  ...
  const isSafeUrl = (url: string) => {
    // http/https 프로토콜만 허용하며, javascript: 또는 data:는 포함할 수 없음
  ...
  /**
   * messageInputDom 입력 상자의 내용 유형 가져오기
  ...
  const insertNodeAtRange = (type: MsgEnum, dom: any, _target: HTMLElement, sr: SelectionRange) => {
    ...
    // 노드를 범위의 맨 앞에 추가
    if (type === MsgEnum.AIT) {
      // @노드에 표시 이름과 uid를 저장하여 나중에 atUidList 추출 시 구조화된 데이터에 직접 의존하여 오판 방지
      ...
      spanNode.contentEditable = 'false' // 편집 불가능으로 설정
      ...
      // 커서를 Range의 끝으로 접기 (true는 Range의 시작 위치로 접기, false는 Range의 끝 위치로 접기)
      ...
      if (preReplyNode) {
        ...
        // 입력 상자에 이미 내용이 있는 경우 커서 위치가 올바른지 확인해야 함
        if (hasChildNodes) {
          ...
          // 입력 상자에 내용이 없으면 나중에 편집하기 쉽도록 공백 노드 추가
          ...
          afterRange.collapse(false) // 종료 위치로 접기
      ...
      divNode.id = 'AIDiv' // id를 replyDiv로 설정
      divNode.contentEditable = 'false' // 편집 불가능으로 설정
      ...
      cursor: default;
      outline: none; /* focus 시 윤곽선 제거 */
    `
      // dom의 value 값을 답장 메시지의 작성자로, dom의 content를 답장 메시지의 내용으로 사용
      const author = dom.name
      // img 태그 노드를 생성하여 아바타로 사용
      ...
      object-fit: contain;
      `
      // div 태그 노드를 생성하여 답장 메시지의 헤더로 사용
    ...
    `
      ...
      flex-shrink: 0;
      user-select: none;
      pointer-events: auto; /* 닫기 버튼 클릭 가능하도록 설정 */
    `
  ...
  function createReplyDom(dom: { accountName: string; content: string; avatar: string }) {
    ...
    replyNode.contentEditable = 'false' // 편집 불가능으로 설정
    ...
    if (Array.isArray(content)) {
      ...
      // data:image/로 시작하는 첫 번째 이미지 가져오기
      content = content.find((item: string) => item.startsWith('data:image/'))
      reply.value.imgCount = imageCount
    }

    // todo: 당분간 http로 시작하는 이미지로 판단, 추후 최적화 필요
    if (content.startsWith('http')) {
      // img 태그 노드를 하나 더 생성하고 src 속성을 base64 인코딩된 이미지로 설정
      ...
      // 본문을 span 태그에 넣고 span 태그 스타일 설정
  ...
  const imgPaste = async (file: any, dom: HTMLElement) => {
    ...
    if (typeof file === 'string' && file.startsWith('blob:')) {
      ...
      // MsgInput 컴포넌트가 노출하는 lastEditRange 가져오기
  ...
  const FileOrVideoPaste = async (file: File) => {
    ...
    if (file.size > 1024 * 1024 * 50) {
      window.$message.warning('파일 크기는 50M를 초과할 수 없습니다. 다시 선택해주세요')
      return
    }
  ...
  const openMsgSession = async (uid: string, type: number = 2) => {
    ...
    if (!existingSession) {
      ...
      // 세션이 존재하지 않으면 세션 목록을 다시 가져와야 하지만 현재 선택된 세션은 유지
  ...
  const processFiles = async (
    ...
    for (const file of files) {
      ...
      // mime 타입이 아닌 경우에 대한 처리를 위해 includes 추가
