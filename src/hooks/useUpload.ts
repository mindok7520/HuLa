...
export interface UploadOptions {
  ...
  /** 분할 업로드 사용 여부 (Qiniu 클라우드에만 유효) */
  useChunks?: boolean
  /** 분할 크기 (단위: 바이트, 기본값 4MB) */
  chunkSize?: number
...
const DEFAULT_CHUNK_SIZE = 4 * 1024 * 1024 // 기본 분할 크기: 4MB
const QINIU_CHUNK_SIZE = 4 * 1024 * 1024 // Qiniu 클라우드 분할 크기: 4MB
const CHUNK_THRESHOLD = 4 * 1024 * 1024 // 4MB, 이 크기를 초과하는 파일은 분할 업로드 사용
...
export const useUpload = () => {
  ...
  const calculateFileHash = async (file: File): Promise<string> => {
    ...
    try {
      console.log('MD5 해시값 계산 시작, 파일 크기:', file.size, 'bytes')
      ...
      console.log(`MD5 계산 완료, 소요 시간: ${duration}ms, 해시값: ${hash}`)
      return hash.toLowerCase()
    } catch (error) {
      const endTime = performance.now()
      const duration = (endTime - startTime).toFixed(2)
      console.error(`파일 해시값 계산 실패, 소요 시간: ${duration}ms:`, error)
  ...
  const getFileType = (fileName: string): string => {
    ...
    // 이미지 유형의 경우 통일된 getMimeTypeFromExtension 함수 사용
  ...
  const uploadToDefaultWithChunks = async (url: string, file: File) => {
    ...
    try {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        if (!response.ok) {
          throw new Error(`분할 ${i + 1}/${totalChunks} 업로드 실패: ${response.statusText}`)
        }
        ...
        console.log(`분할 ${i + 1}/${totalChunks} 업로드 성공, 진행률: ${progress.value}%`)
      }
      ...
      isUploading.value = false
      console.error('기본 스토리지 분할 업로드 실패:', error)
      throw error
    }
  }
  ...
  /**
   * 파일을 분할하여 Qiniu 클라우드에 업로드
   * @param file 파일
   * @param qiniuConfig Qiniu 설정
   * @param chunkSize 분할 크기 (바이트)
  ...
  const uploadToQiniuWithChunks = async (
    ...
    try {
      ...
      // Qiniu 분할 업로드 API v2 사용 - 업로드 블록 생성
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        if (!blockResponse.ok) {
          ...
          console.error(`분할 ${i + 1}/${totalChunks} 업로드 실패:`, {
            status: blockResponse.status,
            statusText: blockResponse.statusText,
            errorText
          })
          throw new Error(`분할 ${i + 1}/${totalChunks} 업로드 실패: ${blockResponse.statusText}`)
        }
        ...
        console.log(`분할 ${progressInfo.uploadedChunks}/${progressInfo.totalChunks} 업로드 성공:`, {
      ...
      if (!completeResponse.ok) {
        throw new Error(`분할 업로드 완료 실패: ${completeResponse.statusText}`)
      }

      const completeResult = await completeResponse.json()
      console.log('분할 업로드 완료:', completeResult)
      ...
      console.error('Qiniu 분할 업로드 실패:', error)
      return { error: 'Upload failed' }
    }
  }
   ...
   * 파일 파싱
   * @param file 파일
   * @param addParams 추가 매개변수
   * @returns 파일 크기, 파일 유형, 파일명, 파일 확장자 등
   */
  const parseFile = async (file: File, addParams: Record<string, any> = {}) => {
    ...
    // TODO: 여기서 유형 판단을 할 필요가 없을 수도 있음, baseInfo를 바로 반환 가능
    ...
    if (currentProvider.value === UploadProviderEnum.QINIU) {
      try {
        ...
        if (file.size > CHUNK_THRESHOLD) {
          console.log('uploadFile - 분할 업로드 방식 사용')
  ...
  /**
   * 업로드 및 다운로드 URL 가져오기
   * 기본 업로드 방식인 경우 업로드 및 다운로드 URL을 가져와서 업로드를 실행
   * Qiniu 업로드 방식인 경우 Qiniu 토큰을 가져오지만, 업로드는 실행하지 않음
   * @param path 파일 경로
   * @param options 업로드 옵션
   */
  const getUploadAndDownloadUrl = async (
    ...
    if (currentProvider.value === UploadProviderEnum.QINIU) {
      try {
        ...
        throw new Error('업로드 자격 증명 가져오기 실패, 다시 시도해 주세요')
      }
    }
  ...
  const doUpload = async (path: string, uploadUrl: string, options?: any): Promise<{ qiniuUrl: string } | string> => {
    ...
    if (uploadUrl === UploadProviderEnum.QINIU && options) {
      ...
      try {
        ...
        isUploading.value = false
        trigger('fail')
        console.error('Qiniu 업로드 실패:', error)
        throw new Error('파일 업로드 실패, 다시 시도해 주세요')
      }
    } else {
      // 기본 업로드 방식 사용
