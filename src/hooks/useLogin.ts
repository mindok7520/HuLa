...
export const useLogin = () => {
   ...
   * 주의: useRouter()는 반드시 setup 컨텍스트 내에서 호출되어야 함
   * 비동기 콜백 내에서는 Vue 컴포넌트 컨텍스트를 잃기 때문에 useRouter()를 호출할 수 없음
   * 따라서 여기서 router 인스턴스를 미리 확보하고 저장하여 후속 비동기 작업에 사용
   */
  let router: ReturnType<typeof useRouter> | null = null
  try {
    router = useRouter()
  } catch (e) {
    console.warn('[useLogin] router 인스턴스를 가져올 수 없습니다. 컴포넌트 컨텍스트 내가 아닐 수 있습니다:', e)
  }
  ...
  const setLoginState = async () => {
    // 로그인 성공 후 로컬 저장소의 wsLogin 삭제. 사용자가 QR 코드 페이지에서 QR 코드를 새로 고침했지만 사용하지 않아 만료되거나 로그인 실패하는 것을 방지
  ...
  const resetLoginState = async (isAutoLogin = false) => {
    ...
    // 1. 로컬 저장소 정리
    if (!isAutoLogin) {
      // TODO 나중에 계정을 구분하여 다른 account로 전환하거나 다른 REFRESH_TOKEN으로 호출해야 함
  ...
  const runFullSync = async (preserveSession?: string) => {
    ...
    // 유지해야 할 세션이 있고 해당 세션이 목록에 계속 존재하는 경우 선택 상태 복원
    if (preserveSession) {
  ...
  const runIncrementalSync = async (preserveSession?: string) => {
    // 세션 목록의 최신 메시지와 읽지 않은 수 우선 보장: 세션을 가져오면 읽지 않음/최신 메시지가 준비됨
    await chatStore.getSessionList(true)
    // 유지해야 할 세션이 있고 해당 세션이 목록에 계속 존재하는 경우 선택 상태 유지
    if (preserveSession) {
      const sessionExists = chatStore.sessionList.some((s) => s.roomId === preserveSession)
      if (!sessionExists) {
        // 세션이 존재하지 않으므로 선택 해제
    ...
    // 유지할 세션이 없는 경우에도 현재 상태 유지 (증분 동기화는 초기화하지 않음)

    // 백그라운드 메시지 동기화: 로그인 명령이 이미 한번 전체/오프라인 동기화를 트리거했으므로 중복 가져오기 방지; 필요할 때만 명시적으로 호출
    // 메시지 프리패치 및 기타 워밍업을 백그라운드에 배치하여 UI 차단 방지
    await Promise.allSettled([
      chatStore.setAllSessionMsgList(20),
      groupStore.setGroupDetails(),
      cachedStore.getAllBadgeList()
    ]).catch((error) => {
      console.warn('[useLogin] 증분 워밍업 작업 실패:', error)
    })
  }

  const init = async (options?: { isInitialSync?: boolean }) => {
    const emojiStore = useEmojiStore()
    // 현재 선택한 세션을 저장하여 동기화 시 사용자의 선택 상태가 손실되지 않도록 함
    ...
    // 이모티콘 목록 초기화 및 백그라운드에서 로컬 캐시 미리 가져오기 (worker + 동시성 제한 사용)
    ...
    // 로그인 직후 이모티콘 로컬 캐시 워밍업 (비동기, 후속 프로세스 차단 안 함)
    void emojiStore.prefetchEmojiToLocal().catch((error) => {
      console.warn('[login] 이모티콘 캐시 워밍업 실패:', error)
    })
    ...
    if (isInitialSync) {
      ...
      try {
        ...
        // 증분 로그인은 세션 준비만 기다리고 힌트를 닫으며, 백그라운드 동기화는 계속됨
        chatStore.syncLoading = false
      }
    }
    // 강제 지속성
  ...
  const routerOrOpenHomeWindow = async () => {
    if (isDesktop()) {
      ...
      // 홈 창이 성공적으로 생성되고 로그인된 경우에만 트레이 메뉴 표시
  ...
  const normalLogin = async (
    ...
    // auto 매개변수에 따라 어디서 로그인 정보를 가져올지 결정
    ...
    invoke('login_command', {
      ...
      .then(async (_: any) => {
        ...
        if (isMobile()) {
          await init()
          await invoke('hide_splash_screen') // 초기화 완료 후 시작 페이지 닫기
        }
      ...
      .catch((e: any) => {
        ...
        // 자동 로그인 실패 시 수동 로그인 인터페이스로 전환하고 버튼 상태 재설정
        if (auto) {
          uiState.value = 'manual'
          loginDisabled.value = false
          loginText.value = t('login.button.login.default')
          // 자동 로그인 취소
          settingStore.setAutoLogin(false)
          // 이전에 로그인 시도했던 계정 정보를 수동 로그인 양식에 자동 채우기
