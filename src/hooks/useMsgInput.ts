...
export const useMsgInput = (messageInputDom: Ref) => {
  ...
  /** 현재 선택된 AI 옵션 key 기록 */
  // /로 패널을 닫을 때 현재 선택 항목을 지워야 하므로 null 허용
  ...
  const personList = computed(() => {
    if (aitKey.value && !isChinese.value) {
      return groupStore.userList.filter((user) => {
        // 그룹 닉네임(myName)과 원래 이름(name) 동시 매칭
        ...
        try {
          ...
          if (!imageProcessed) {
            ...
            if (content) {
              ...
              // 이미지도 텍스트도 없을 때 팁 표시
              alert('현재 클립보드에서 해당 유형의 내용을 가져올 수 없습니다. ctrl/command + v를 사용해주세요.')
            }
          }
        } catch (error) {
  ...
  const extractAtUserIds = (content: string, userList: BaseUserItem[]): string[] => {
    ...
    // @ 태그 노드를 통해 우선 추출
    // uid가 있는 @ 노드를 우선 읽어 실제로 선택된 멤버만 통계에 포함되도록 보장
    ...
    // 더 정확한 정규식을 사용하여 @ 사용자 매칭
    // 공백 문자나 문자열 끝을 만날 때까지 @ 뒤의 비공백 문자 매칭
    const regex = /@([^\s]+)/g
  ...
  /** html 태그 제거 (답장 시 입력 내용이 있는지 식별하는 데 사용) */
  const stripHtml = (html: string) => {
    try {
      ...
      // 붙여넣은 이미지(temp-image id가 있는 이미지 요소)가 포함되어 있는지 확인
  ...
  /** 메시지 전송 이벤트 처리 */
  // TODO 메시지를 전환할 때 입력 상자의 내용을 기록해야 함 (nyh -> 2024-03-01 07:03:43)
  const send = async () => {
    ...
    if (!messageStrategy) {
      window.$message.warning('현재 해당 유형의 메시지 전송을 지원하지 않습니다')
      return
    }
    ...
    try {
      ...
      if (msg.type === MsgEnum.IMAGE || msg.type === MsgEnum.EMOJI) {
        // TODO: 기본 업로드 방식을 사용하는 경우 uploadFile 메서드가 업로드 및 다운로드 링크를 반환하지만, Qiniu 업로드 방식을 사용하는 경우 doUpload 메서드를 호출해야 해당 다운로드 링크가 반환됨
        ...
        // 메시지 본문의 URL을 서버 URL로 업데이트(Qiniu인지 기본 업로드 방식인지 판단), provider가 없으면 기본적으로 downloadUrl 할당
        messageBody.url =
  ...
  /** 입력 상자에 수동으로 값을 입력할 때 input 이벤트 트리거 (vueUse의 디바운스 사용) */
  ...
  const inputKeyDown = async (e: KeyboardEvent) => {
    ...
    // 현재 시스템이 mac인 경우, 현재 chat.value.sendKey가 Enter인지 판단하고, 현재 ⌘+Enter를 눌렀는지 판단해야 함
  ...
  const handleAit = (item: BaseUserItem) => {
    // 병음 입력 중이면 메시지 전송 안 함
    if (isChinese.value) {
      return
    }
    // 먼저 입력 상자에 포커스 확인
    focusOn(messageInputDom.value)
    // 먼저 현재 에디터 범위 가져오기 및 저장
  ...
  const handleAI = (_item: any) => {
    ...
    // TODO: (임시 표시) AI 연동 중 팁 표시
    window.$message.info('현재 AI 연동 중입니다. 기대해주세요')
    // AI 선택 팝업 닫기
    aiDialogVisible.value = false
  ...
  onMounted(async () => {
    ...
    useMitt.on(MittEnum.REPLY_MEG, (event: any) => {
      ...
      try {
        ...
        // 1단계: 입력 상자가 먼저 포커스를 받도록 함
        focusOn(messageInputDom.value)

        // 2단계: 기존 답장 상태를 완전히 정리
        // 이미 답장 메시지가 있는 경우 기존 답장 상자를 먼저 제거해야 함
        const existingReplyDiv = document.getElementById('replyDiv')
        if (existingReplyDiv) {
          existingReplyDiv.remove()
        }

        // 항상 reply 상태를 재설정하여 이전 답장 상태가 완전히 지워지도록 함
        reply.value = { avatar: '', imgCount: 0, accountName: '', content: '', key: 0 }
  ...
  const sendFilesDirect = async (files: File[]) => {
    ...
    // 모든 파일 완료 대기 (UI 차단 안 함, 파일이 하나씩 성공 표시됨)
    await Promise.allSettled(tasks)

    // 입력 상자에 자동 전송이 필요한 이미지가 있는지 확인
    try {
      await nextTick()
      if (
