...
/**
 * 전역 단축키 관리 Hook
 * 전역 단축키 등록, 등록 해제 및 관리를 담당
 * 구성 기반 방식을 사용하여 새로운 단축키 확장이 용이함
 */
export const useGlobalShortcut = () => {
  ...
  /**
   * capture 창 존재 여부 확인
   * 없으면 생성하고, 있으면 닫기 차단이 설정되어 있는지 확인
   */
  const ensureCaptureWindow = async () => {
    ...
    if (captureWindow) {
      ...
      captureWindow.onCloseRequested(async (event) => {
        ...
        // 초기화 이벤트 트리거, Screenshot 컴포넌트 재초기화
  ...
  const handleScreenshot = async () => {
    try {
      ...
      // 창을 전체 화면으로 설정 (메뉴바 포함)
      const screenWidth = window.screen.width * window.devicePixelRatio
      const screenHeight = window.screen.height * window.devicePixelRatio

      // 창 레벨 설정에 의존하여 메뉴바를 덮도록 함
  ...
  const handleOpenMainPanel = async () => {
    ...
    try {
      ...
      console.log(`단축키 트리거 - 창 상태: 표시=${isVisible}, 최소화=${isMinimized}`)

      if (isVisible && !isMinimized) {
        // 창이 현재 표시되어 있고 최소화되지 않았으면 바로 숨김
        await homeWindow.hide()
      } else {
        // 최소화 상태 처리
        if (isMinimized) {
  ...
  /**
   * 단축키 등록 해제
   * @param shortcut 등록 해제할 단축키 문자열
   */
  const unregisterShortcut = async (shortcut: string) => {
    try {
      await unregister(shortcut)
      console.log(`단축키 등록 해제 성공: ${shortcut}`)
    } catch (error) {
      console.error(`단축키 등록 해제 실패: ${shortcut}`, error)
    }
  }
  ...
  const forceCleanupShortcuts = async (shortcuts: string[]) => {
    for (const shortcut of shortcuts) {
      try {
        ...
        console.log(`강제 정리 ${shortcut} (미등록 가능성 있음)`)
      }
    }
  }
  ...
  const handleShortcutUpdate = async (config: ShortcutConfig, newShortcut: string) => {
    ...
    console.log(`[Home] settings 창에 단축키 상태 업데이트 알림 완료 [${config.key}]: ${success ? '등록됨' : '미등록'}`)
  }
  ...
  const handleGlobalShortcutToggle = async (enabled: boolean) => {
    if (enabled) {
      // 켜져 있을 때 모든 단축키 재등록 및 설정 페이지에 알림
      ...
      // 꺼져 있을 때 모든 단축키 등록 해제 및 설정 페이지에 미바인딩 상태 알림
      for (const config of shortcutConfigs) {
        const savedShortcut = settingStore.shortcuts?.[config.key] || config.defaultValue

        // 설정 페이지에 등록 상태 업데이트를 미바인딩으로 알림
        await emitTo('settings', config.registrationEventName, {
          shortcut: savedShortcut,
          registered: false
        })
      ...
      listen(config.updateEventName, (event) => {
        ...
        if (newShortcut) {
          ...
          if (globalEnabled) {
            ...
            console.log(`[Home] 전역 단축키가 꺼져 있음, 단축키 업데이트 건너뜀 [${config.key}]`)
          }
        } else {
