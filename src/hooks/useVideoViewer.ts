...
export const useVideoViewer = () => {
  ...
  //   // 파일명 본문에 사용할 수 있는 길이 계산 (확장자와 줄임표 길이 제외)
  ...
  const getAllMediaFromChat = (url: string, includeTypes: MsgEnum[] = [MsgEnum.VIDEO]) => {
    ...
    messages.forEach((msg) => {
      if (includeTypes.includes(msg.message?.type) && msg.message.body?.url) {
        ...
        // 요소 추가 후 대상 URL 인지 판단
        if (isTarget) {
          currentIndex = mediaUrls.length - 1 // 배열 마지막 인덱스 사용
        }
      }
    })
    return {
   ...
   * @param customVideoList 사용자 정의 비디오 목록 (채팅 기록 등)
   */
  const openVideoViewer = async (
    ...
    // 현재 비디오의 인덱스가 처리된 목록에서 어디에 있는지 찾기
    ...
    // 단일 비디오 모드를 구분하지 않고 목록 모드로 통일하여 사용
