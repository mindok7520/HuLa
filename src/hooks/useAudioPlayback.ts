...
export const useAudioPlayback = (
  ...
  const hasBeenPlayed = ref(false) // 재생된 적이 있는지 여부
  ...
  const handleAudioStateChange = () => {
    ...
    // 현재 재생 중인 오디오가 이 컴포넌트의 오디오가 아니면 재생 상태 초기화
  ...
  const cleanup = () => {
    ...
    if (audioElement.value) {
      try {
        ...
        audioElement.value.load() // 오디오 요소 초기화
      } catch (error) {
