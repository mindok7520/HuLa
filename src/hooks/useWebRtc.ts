...
const loadIceServers = async () => {
  try {
    ...
    if (ice && Array.isArray(ice.urls) && ice.urls.length > 0) {
      ...
      info('ICE 구성이 비어 있어 기본 내장 구성을 사용합니다')
    }
  } catch (e) {
...
export const useWebRtc = (roomId: string, remoteUserId: string, callType: CallTypeEnum, isReceiver: boolean) => {
  ...
  const getLocalStream = async (type: CallTypeEnum) => {
    try {
      ...
      // localStream 정보 출력 (스트림을 직접 직렬화할 수 없음, 그렇지 않으면 null 반환)
      ...
      window.$message.error('로컬 미디어 스트림을 가져오지 못했습니다. 장치를 확인하십시오!')
      error(`로컬 미디어 스트림을 가져오지 못했습니다. 장치를 확인하십시오! ${err}`)
  ...
  const sendOffer = async (offer: RTCSessionDescriptionInit) => {
    try {
      ...
      console.error('SDP offer 전송 실패:', error)
    }
  }
  ...
  const sendIceCandidate = async (candidate: RTCIceCandidate) => {
    try {
      ...
      console.error('ICE 후보 전송 실패:', error)
    }
  }
  ...
  const handleOffer = async (signal: RTCSessionDescriptionInit, video: boolean, roomId: string) => {
    try {
      ...
      if (!roomId) {
        window.$message.error('방 번호가 존재하지 않습니다. 다시 연결해 주십시오!')
        return false
      }
  ...
  const sendAnswer = async (answer: RTCSessionDescriptionInit) => {
    try {
      ...
      console.error('SDP answer 전송 실패:', error)
    }
  }
    ...
    if (localStream.value) {
      ...
      if (videoTrack) {
        ...
        if (!videoTrack.enabled) {
          console.log('로컬 비디오 꺼짐, 상대방이 볼 수 없음')
        } else {
          console.log('로컬 비디오 켜짐, 상대방이 볼 수 있음')
        }
      } else if (callType === CallTypeEnum.VIDEO) {
        // 비디오 트랙이 없지만 영상 통화인 경우 재시도
        try {
          const constraints = {
            audio: false,
  ...
  const startScreenShare = async () => {
    try {
      ...
      const oldVideoTrack = localStream.value.getVideoTracks()[0]
      if (!newVideoTrack) {
        window.$message.error('화면 공유 실패. 권한 설정을 확인하십시오!')
        return
      }
  ...
  // WebRTC 시그널링 메시지 수신 (등록 및 언로드 함수 저장)
  ...
  onMounted(async () => {
    ...
    if (!isReceiver) {
      console.log(`호출자가 ${callType === CallTypeEnum.VIDEO ? '영상' : '음성'} 통화 요청을 보냄`)
