...
// 동일한 unlisten 함수에 대한 중복 호출로 인해 기본 listeners[eventId]가 존재하지 않는 것을 방지
...
/** tauri Listener 이벤트 리스너를 자동으로 관리하는 hooks */
export const useTauriListener = () => {
  ...
  const setupWindowCloseListener = async () => {
    try {
      ...
      if (currentWindowLabel !== 'home') {
        info(`[useTauriListener]현재 창 라벨 닫기 리스너 설정: ${currentWindowLabel}`)
  ...
  return {
    addListener,
    pushListeners,
    // 비 컴포넌트 환경에서 수동으로 정리할 수 있도록 정리 메서드 노출
    cleanup
  }
}
