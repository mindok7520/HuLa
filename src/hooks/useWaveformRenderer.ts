...
export const useWaveformRenderer = (
  ...
  /**
   * 오디오 길이에 따른 파형 너비 계산 (최소 10px, 최대 100px)
   */
  ...
  const generateWaveformData = async (input: ArrayBuffer | Uint8Array | SharedArrayBuffer) => {
    try {
      ...
      // arrayBuffer가 ArrayBuffer 타입인지 확인
      let arrayBuffer: ArrayBuffer
      if (input instanceof Uint8Array) {
        // buffer가 ArrayBuffer이면 직접 사용, 그렇지 않으면 새 ArrayBuffer 생성
  ...
  const drawWaveform = () => {
    ...
    // 업데이트가 필요하거나 캐시가 없을 때만 정적 파형 재생성
    if (needUpdateCache) {
      // 오프스크린 캔버스 생성하여 정적 파형 렌더링
  ...
  // 재생 진행 변화 감지하여 파형 다시 그리기 (스로틀 처리)
