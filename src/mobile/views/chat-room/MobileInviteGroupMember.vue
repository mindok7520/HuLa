...
// 검색 기능
const doSearch = () => {
  // 검색 로직은 filteredContacts에 이미 구현됨
}

// 초대 처리
const handleInvite = async () => {
  if (selectedList.value.length === 0) {
    window.$message.warning('초대할 친구를 선택해주세요')
    return
  }
...
onMounted(async () => {
  try {
    // groupStore.allUserInfo에 데이터가 있도록 모든 그룹의 멤버 데이터 먼저 로드
    ...
    // 모든 사용자 정보를 얻기 위해 다른 그룹의 멤버 목록 로드
    await Promise.all(
      groupSessions
