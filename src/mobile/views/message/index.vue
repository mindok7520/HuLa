...
const isEnablePullRefresh = ref(true) // 당겨서 새로고침 활성화 여부, 현재 맨 위로 스크롤될 때만 활성화됨
...
const allUserMap = computed(() => {
  const map = new Map<string, any>() // User는 정의된 사용자 타입
...
const sessionList = computed(() => {
  return (
    chatStore.sessionList
      ...
      // 정렬 로직 추가: 상단 고정 상태 우선 정렬, 그 다음 활동 시간순 정렬
      .sort((a, b) => {
        // 1. 상단 고정 상태 우선 정렬 (상단 고정 항목이 앞쪽)
        if (a.top && !b.top) return -1
        if (!a.top && b.top) return 1

        // 2. 동일한 상단 고정 상태 내에서, 마지막 활동 시간 내림차순 정렬 (최신 항목이 앞쪽)
...
const onRefresh = () => {
  // 0.5초 미만이면 0.5초 지연, API 실행 시간이 0.5초를 초과하면 getSessionList 시간 기준
  loading.value = true
  count.value++
...
const intoRoom = (item: any) => {
  ...
  setTimeout(() => {
    // 사용자를 찾으면 해당 세션이 친구의 것임을 의미하므로 친구 uid 전달; 동시에 id가 1인 HuLa 도우미 제외
...
const handleLongPress = (e: PointerEvent, item: SessionItem) => {
  ...
  const setLongPressMenuTop = () => {
    ...
    const currentTarget = target.closest('.grid') // 상위로 탐색하여 grid를 찾으면 중지
...
.keyboard-mask {
  ...
  z-index: 1400; // Naive 팝업 레이어보다 낮고 페이지 콘텐츠보다 높음
  pointer-events: auto; // 이벤트 수신 가능 확인
  touch-action: none; // 스크롤 금지
}

::deep(#search) {
  position: relative;
  z-index: 1500; // 키보드 마스크보다 높고 Naive 팝업 레이어보다 낮음
}
</style>
