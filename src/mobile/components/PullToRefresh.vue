  ...
  <div
    ...
    <div
      ...
      <template v-if="isRefreshing">
        ...
        <span class="ml-2 text-sm color-#333">새로고침 중...</span>
      </template>
      <template v-else>
        <div class="flex-center flex-col">
          ...
          <span class="text-xs mt-1">
            {{ distance >= threshold ? '놓아서 새로고침' : '당겨서 새로고침' }}
          </span>
        </div>
      </template>
    </div>
...
type Props = {
  threshold?: number // 새로고침 트리거 임계값
  indicatorHeight?: number // 표시기 높이
  disabled?: boolean // 당겨서 새로고침 비활성화 여부
}
