<template>
  <n-flex vertical class="select-none">
      ...
      <template #default="{ item }">
        <div class="flex gap-2 w-full text-14px mb-15px">
          ...
          <div
            ...
            <span class="text-(12px #909090)" v-else-if="item.status === RequestNoticeAgreeStatus.IGNORE">무시됨</span>
            <span
    ...
    <n-flex v-if="applyList.length === 0" vertical justify="center" align="center" class="py-40px">
      <n-empty :description="props.type === 'friend' ? '친구 신청 없음' : '그룹 알림 없음'" />
    </n-flex>
  </n-flex>
</template>
...
const applyMsg = computed(() => (item: any) => {
  if (props.type === 'friend') {
    return isCurrentUser(item.senderId) ? (isAccepted(item) ? '요청을 수락했습니다' : '초대를 확인 중입니다') : '친구 요청'
  } else {
...
const loadMoreFriendRequests = async () => {
  // 이미 마지막 페이지이거나 로딩 중이면 더 이상 로드하지 않음
