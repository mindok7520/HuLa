            ...
            <div class="text-10px absolute inset-0 flex ps-2 items-center justify-around text-white font-medium">
              <span class="flex items-center">
                <div v-if="(userStore.userInfo?.itemIds?.length ?? 0) > 0">
                  ...
                  <span class="font-bold">개 뱃지</span>
                </div>
                <span v-else>아직 뱃지가 없어요~</span>
              </span>
...
const toChatRoom = async () => {
  try {
    ...
    if (!existingSession) {
      ...
      // 세션이 존재하지 않으면 세션 목록을 다시 가져오되, 현재 선택된 세션 유지
...
// 사용자 상세 정보, 기본 필드만 작성, 추가하지 않으면 undefined 에러 발생 가능
    ...
    .then(async () => {
      if (userDetailInfo.value?.uid) {
        try {
          ...
          window.$message.success('친구 삭제 완료')
          router.back()
        } catch (error) {
        ...
        window.$message.warning('친구를 찾을 수 없어요')
      }
    })
    .catch(() => {
      // 사용자가 취소를 클릭함, 아무 작업도 하지 않음
    })
}
...
function enter(el: Element, done: () => void) {
  ...
  box.addEventListener(
    'transitionend',
    () => {
      box.style.height = 'auto' // 애니메이션 종료 후 auto로 설정하여 레이아웃 영향 방지
      done()
    },
    { once: true }
  )
