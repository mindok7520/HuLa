...
interface MobileLayoutProps {
  ...
  /** 상단 안전 영역의 사용자 정의 CSS class */
  topSafeAreaClass?: string
  /** 하단 안전 영역의 사용자 정의 CSS class */
...
const addFileToStore = (data: MessageType) => {
  ...
  try {
    ...
    // 유효한 URL이 아닌 경우 메시지 ID를 파일명으로 사용
...
useMitt.on(WsResponseMessageType.RECEIVE_MESSAGE, async (data: MessageType) => {
  ...
  // 이미지 또는 비디오 메시지인 경우 file store에 추가
  addFileToStore(data)
      ...
      if (!isMobile()) {
        try {
          ...
          if (home) {
            ...
            // 창이 보이지 않거나, 최소화되었거나, 포커스가 없으면 효과음 재생
            shouldPlaySound = !isVisible || isMinimized || !isFocused
          } else {
            // home 창을 찾을 수 없으면 효과음 재생
