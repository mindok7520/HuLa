...
/**
 * 메시지 전략 추상 클래스, 모든 메시지 전략은 이 인터페이스를 구현해야 합니다.
 */
abstract class AbstractMessageStrategy implements MessageStrategy {
  ...
  uploadFile(
    ...
    throw new AppException('해당 메시지 유형은 파일 업로드를 지원하지 않습니다')
  }
...
class TextMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  getMsg(msgInputValue: string, replyValue: any): any {
    ...
    if (msg.content.length > 500) {
      throw new AppException('메시지 내용이 제한 500자를 초과했습니다. 나누어서 보내주세요')
    }
    return msg
  }
...
class ImageMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  async uploadFile(
    ...
    // URL인 경우 원본 URL을 다운로드 링크로 직접 반환함
    if (this.isImageUrl(path)) {
      return {
        uploadUrl: '', // 업로드 URL 필요 없음
        downloadUrl: path // 원본 URL 직접 사용
      }
    }

    // useUpload 훅을 사용하여 업로드 및 다운로드 URL 가져오기
    ...
    try {
      ...
      throw new AppException('업로드 링크를 가져오지 못했습니다. 다시 시도해 주세요')
    }
  }
  ...
  async doUpload(path: string, uploadUrl: string, options?: any): Promise<{ qiniuUrl?: string } | void> {
    ...
    try {
      ...
      // 칠우클라우드(Qiniu) 업로드인 경우 qiniuUrl 반환
      ...
      throw new AppException('파일 업로드에 실패했습니다. 다시 시도해 주세요')
    }
  }
...
class LocationMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  getMsg(msgInputValue: string, replyValue: any): any {
    try {
      ...
      if (!locationData.latitude || !locationData.longitude || !locationData.address) {
        throw new AppException('유효하지 않은 위치 데이터입니다. 필수 필드가 누락되었습니다')
      }
      ...
      return {
        ...
        precision: locationData.precision || '고정밀',
  ...
  buildMessageBody(msg: any, reply: any): any {
    return {
      ...
      replyMsgId: msg.reply?.key || undefined, // 답장 메시지 ID
      reply: reply.value.content
        ? {
          body: reply.value.content,
          id: reply.value.key,
...
class FileMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  private async getFileFromPath(path: string): Promise<File> {
    try {
      ...
      throw new AppException('파일을 읽을 수 없습니다. 파일이 존재하는지 확인해 주세요')
    }
  }
  ...
  async getMsg(msgInputValue: string, replyValue: any, fileList?: File[]): Promise<any> {
    ...
    // fileList의 파일을 우선적으로 사용함
    if (fileList && fileList.length > 0) {
      file = fileList[0]
    } else {
      // msgInputValue에서 파일 경로를 해석해봄
      const path = parseInnerText(msgInputValue, 'temp-file')
      if (!path) {
        throw new AppException('보낼 파일을 선택해 주세요')
      }
  ...
  async uploadFile(
    ...
    try {
      ...
      throw new AppException('파일 업로드 링크를 가져오지 못했습니다. 다시 시도해 주세요')
    }
  }
...
class EmojiMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  getMsg(msgInputValue: string, replyValue: any): any {
    ...
    if (!this.isValidEmojiUrl(msgInputValue)) {
      throw new AppException('유효하지 않은 이모티콘 URL입니다')
    }
  ...
  // 이모티콘은 실제 업로드가 필요하지 않으며 원본 URL을 직접 반환함
  async uploadFile(
    path: string,
    options?: { provider?: UploadProviderEnum }
  ): Promise<{ uploadUrl: string; downloadUrl: string; config?: any }> {
    console.log('이모티콘 원본 URL 사용:', path, options)
    return {
      uploadUrl: '', // 업로드 URL 필요 없음
      downloadUrl: path // 원본 URL 직접 사용
    }
  ...
  // 이모티콘은 실제 업로드가 필요하지 않으므로 이 메서드는 빈 구현임
  async doUpload(path?: string, uploadUrl?: string, options?: any): Promise<void> {
    console.log('이모티콘은 업로드가 필요하지 않으므로 업로드 단계를 건너뜁니다', path, uploadUrl, options)
...
class VideoMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  private async validateVideo(file: File): Promise<File> {
    ...
    if (!this.ALLOWED_TYPES.includes(file.type)) {
      throw new AppException('MP4/MOV/AVI/WMV 형식의 비디오만 지원됩니다')
    }
    // 파일 크기 확인
    if (file.size > this.MAX_UPLOAD_SIZE) {
  ...
  private async convertToVideoFile(videoFile: string | File): Promise<File> {
    ...
    if (videoFile.startsWith('<') || videoFile.includes('src="blob:')) {
      ...
      try {
        ...
        // 4. File 객체로 변환
        const fileName = `video_${Date.now()}.mp4` // 기본 파일 이름
    ...
    try {
      ...
      throw new AppException('비디오 파일을 읽을 수 없습니다. 파일 경로가 올바른지 확인해 주세요')
    }
  }

  // 파일 이름에 따라 비디오 유형 가져오기
  ...
  async uploadThumbnail(
    ...
    try {
      ...
      const uploadOptions: UploadOptions = {
        provider: options?.provider || UploadProviderEnum.QINIU,
        scene: UploadSceneEnum.CHAT,
        enableDeduplication: true // 중복 제거 활성화, 해시값 계산 사용
      }
      ...
      console.error('썸네일 업로드 링크 가져오기 실패:', error)
      throw new AppException('썸네일 업로드 링크를 가져오지 못했습니다. 다시 시도해 주세요')
    }
  }
  ...
  async doUploadThumbnail(
    ...
    try {
      // File 객체를 임시 파일에 쓰고, 기존 doUpload 메서드를 사용함
      const tempPath = `temp-thumbnail-${Date.now()}-${thumbnailFile.name}`

      // File 객체를 ArrayBuffer로 변환한 후 임시 파일에 씀
      ...
      // enableDeduplication 파일 중복 제거 활성화, 해시값 계산 사용
      ...
      throw new AppException('썸네일 업로드에 실패했습니다. 다시 시도해 주세요')
    }
  }
  ...
  buildMessageBody(msg: any, reply: any): any {
    ...
    return {
      ...
      thumbUrl: thumbUrl, // 로컬 미리보기 URL, 업로드 완료 후 서버 URL로 대체됨
  ...
  async uploadFile(
    ...
    try {
      ...
      throw new AppException('비디오 업로드 링크를 가져오지 못했습니다')
    }
  }
  async doUpload(path: string, uploadUrl: string, options?: any): Promise<{ qiniuUrl?: string } | void> {
    if (isVideoUrl(path)) {
      throw new AppException('유효한 비디오 URL인지 확인해 주세요')
    }
...
class UnsupportedMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  getMsg(msgInputValue: string, replyValue: any, fileList?: File[]): any {
    ...
    msgInputValue
    fileList
    throw new AppException('아직 지원되지 않는 형식의 메시지입니다')
  }
...
class VoiceMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  async uploadFile(
    ...
    try {
      ...
      throw new AppException('음성 업로드 링크를 가져오지 못했습니다. 다시 시도해 주세요')
    }
  }
  ...
  async doUpload(path: string, uploadUrl: string, options?: any): Promise<{ qiniuUrl?: string } | void> {
    ...
    try {
      ...
      // Qiniu 업로드인 경우 qiniuUrl 반환
      if (options?.provider === UploadProviderEnum.QINIU) {
        return { qiniuUrl: result as string }
      }
    } catch (_error) {
      throw new AppException('음성 파일 업로드에 실패했습니다. 다시 시도해 주세요')
    }
  }
}
...
class VideoCallMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  getMsg(_msgInputValue: string, callInfo: any): any {
    return {
      ...
      reason: callInfo.reason, // 종료 사유: 시간 초과 / 끊기 / 이상
  ...
  buildMessageBody(msg: any): any {
    return {
      ...
      isGroup: msg.isGroup // 그룹 통화 여부
    }
  }

  // 시스템 메시지는 업로드 작업이 필요 없음
  async uploadFile(): Promise<{ uploadUrl: string; downloadUrl: string; config?: any }> {
...
class AudioCallMessageStrategyImpl extends AbstractMessageStrategy {
  ...
  buildMessageBody(msg: any): any {
    return {
      ...
      reason: msg.reason, // 종료 사유
      startTime: msg.startTime, // 시작 타임스탬프
      endTime: msg.endTime, // 종료 타임스탬프
      creator: msg.creator, // 발신자 UID
      isGroup: msg.isGroup // 그룹 통화 표시
    }
  }

  /**
   * 빈 구현 (시스템 메시지는 파일 업로드가 필요 없음)
   * @returns 항상 빈 업로드 정보를 반환함
   */
  async uploadFile(): Promise<{ uploadUrl: string; downloadUrl: string; config?: any }> {
    return {
      uploadUrl: '',
