  ...
  <div ref="videoContainerRef" :style="containerStyle" @dblclick="handleOpenVideoViewer">
    ...
    <div class="video-overlay">
      ...
      <div
        ...
        <!-- 확인되지 않았거나 다운로드되지 않은 경우 다운로드 아이콘 표시 -->
...
// 비디오 썸네일 실제 로드 크기 (props에 너비/높이가 없는 경우 사용)
const loadedThumbWidth = ref(0)
const loadedThumbHeight = ref(0)

const imageStyle = computed(() => {
  // 로드된 이미지 크기를 우선 보정(더 정확함), 없으면 props의 원본 크기 사용
  ...
  // 고정 크기: 세로와 가로 각각 너비/높이를 고정하여 가로가 정사각/세로형으로 변하는 것 방지
...
const resolveMobilePlayableUrl = async () => {
  ...
  if (!downloaded) return '' // 다운로드되지 않았으면 빈 값 반환
  ...
  if (props.body?.url && !isOpening.value) {
    ...
    try {
      ...
      if (!isDownloaded) {
        ...
        // 다운로드 실패 시 비디오를 열지 않음
        if (!isVideoDownloaded.value) {
          console.error('비디오 다운로드 실패, 열 수 없습니다')
          return
        }
      }
      ...
      if (isMobile()) {
        ...
        if (!playableUrl) {
          console.error('재생 가능한 비디오 주소를 찾을 수 없습니다')
          return
        }
