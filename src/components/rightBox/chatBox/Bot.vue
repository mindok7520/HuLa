...
// 기록 뷰 기록, Markdown과 외부 링크 간 반환 용이
...
// README/Markdown 콘텐츠 필터링: 로컬 파일 신뢰, 레이아웃 변경 최소화; 향후 필터링 필요 시 이 방법 확장 가능
...
// 데스크톱에서만 임베디드 Webview 생성 허용, 동시에 Tauri 내부 컨텍스트 준비 확인
...
const openLocalModel = async () => {
  try {
    ...
    window.$message?.error('모델 파일 선택 실패, 다시 시도해 주세요')
  }
}
...
const createExternalWebview = async (url: string) => {
  // 새 Webview를 현재 창에 부착, 좌표 크기는 컨테이너 기준으로 맞춤
  const windowInstance = await ensureHostWindow()
  if (!windowInstance || !webviewContainer.value) return

  // 새로 고침 후 이전 인스턴스가 남아 있을 수 있으므로 동일한 이름의 Webview 닫기 시도
  try {
...
const loadReadme = async (recordHistory = false, resetHistory = false) => {
  ...
  try {
    ...
    // README 소스 신뢰, 원래 레이아웃 유지 위해 직접 렌더링
    renderedMarkdown.value = sanitizeMarkdown(html)
    // 먼저 뷰 상태 업데이트, nextTick 시 컨테이너 마운트 보장
    currentView.value = { type: 'readme' }
    isViewingLink.value = false
    currentUrl.value = ''

    // DOM 업데이트 후 링크 클릭 리스너 추가
    await nextTick()
    attachLinkListeners()
    finishLoading()
    ...
    renderedMarkdown.value = '<p>로드 실패, 나중에 다시 시도해 주세요</p>'
    if (recordHistory) {
      historyStack.value.pop()
    }
    errorLoading()
...
const loadMarkdownFile = async (filePath: string, recordHistory = true) => {
  ...
  try {
    ...
    // 로컬 Markdown 로드 시 임베디드 페이지도 제거
    await destroyExternalWebview()
    const html = await invoke<string>('parse_markdown', {
      filePath: filePath
    })
    // 로컬 Markdown 신뢰, 원래 레이아웃 유지 위해 직접 렌더링
    renderedMarkdown.value = sanitizeMarkdown(html)

    // iframe 대신 markdown 뷰에 표시
    isViewingLink.value = false
    // 먼저 뷰 상태 업데이트, nextTick 시 컨테이너 마운트 보장
...
const handleLinkClick = async (event: Event) => {
  ...
  if (href.startsWith('#')) {
    ...
    // .md 파일인 경우 Rust 백엔드 사용하여 파싱 및 렌더링
    console.log('markdown 파일 로드:', href)
    await loadMarkdownFile(href, true)
  } else {
    // 다른 모든 링크(외부 링크 및 상대 링크 포함)는 Tauri Webview 임베디드를 통해 열기
...
watch(isViewingLink, async (newValue) => {
  if (!newValue) {
    // README 뷰로 돌아가기, DOM 업데이트 후 리스너 다시 추가 대기
    await nextTick()
    attachLinkListeners()
  } else {
...
// 컴포넌트 마운트 시 로드
onMounted(() => {
  // 새로 고침 후 잔류 임베디드 Webview가 있을 수 있으므로 닫기 시도
