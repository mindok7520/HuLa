...
const shouldShowFloatFooter = computed<boolean>(() => {
  ...
  // 자동 스크롤 또는 더 많은 메시지 로드 시 표시하지 않음
  ...
  // 바닥에 가까우면 어떤 경우에도 표시하지 않아 세션 전환 시 깜박임 방지
  if (distanceFromBottom <= 20) {
    return false
  }

  // 우선순위 1: 새 메시지가 있는 경우 새 메시지 알림 우선 표시
  if (currentNewMsgCount.value?.count && currentNewMsgCount.value.count > 0) {
    return true
  }

  // 우선순위 2: 아래로 스크롤하고 바닥에서 멀리 떨어져 있을 때만 버튼 표시
...
const hoverId = ref('')
// 기록 메시지 로드 중인지 식별하기 위한 플래그 추가
const isLoadingMore = ref(false)
// 초기화 시 상단 로드 자동 트리거 방지
...
const loadTopAnnouncement = async (roomId?: string): Promise<void> => {
  ...
  try {
    ...
    console.error('상단 고정 공지 가져오기 실패:', error)
    if (targetRoomId === currentRoomId.value) {
      topAnnouncement.value = null
    }
  }
...
// 1. 방 전환 감지, 초기화 스크롤 의도 트리거
watch(
  () => [currentRoomId.value] as const,
  ([newRoomId], [oldRoomId]) => {
    // 방 전환 및 DOM 준비 완료 시에만 초기화 의도 트리거
...
// 3. 구체적인 스크롤 작업 실행 - watchPostEffect를 사용하여 DOM 업데이트 완료 보장
watchPostEffect(() => {
...
const jumpToReplyMsg = async (key: string): Promise<void> => {
  ...
  // 로드 상태 표시
  window.$message.info('메시지 찾는 중...')

  // 대상 메시지를 찾거나 더 이상 로드할 수 없을 때까지 기록 메시지 로드 시도
  let foundMessage = false
  let attemptCount = 0
  const MAX_ATTEMPTS = 5 // 무한 루프 방지를 위해 최대 시도 횟수 설정
...
const handleScrollByIntent = (intent: ScrollIntentEnum): void => {
  ...
  switch (intent) {
    case ScrollIntentEnum.INITIAL:
      // 초기화 스크롤: 최신 메시지를 표시하기 위해 바로 바닥으로 스크롤
      scrollToBottom()
      break

    case ScrollIntentEnum.NEW_MESSAGE:
      // 새 메시지 스크롤: 바로 바닥으로 스크롤
      scrollToBottom()
      break

    case ScrollIntentEnum.LOAD_MORE:
      // 더 로드: 스크롤 실행 안 함, handleLoadMore에서 관리
      break
...
const scrollToBottom = (): void => {
  ...
  // 바닥으로 표시하고 자동 스크롤 보호를 활성화하여 스크롤 중 이벤트로 인해 isAtBottom이 잘못 false로 설정되는 것을 방지
  isAtBottom.value = true
...
// 메시지 목록 크기 변경 감지, 현재 바닥에 있으면 자동으로 바닥으로 스크롤
useResizeObserver(messageListRef, () => {
  const container = scrollContainerRef.value
  if (!container) return

  // DOM을 통해 직접 거리 계산, isAtBottom 상태에 완전히 의존하지 않아 상태 업데이트 지연 문제 방지
  const { scrollHeight, scrollTop, clientHeight } = container
  const distanceFromBottom = scrollHeight - scrollTop - clientHeight

  // 바닥 거리가 150px 미만(일정 오차 허용)이거나 상태가 바닥으로 표시된 경우 스크롤 실행
  if (distanceFromBottom <= 150 || isAtBottom.value) {
    // nextTick을 사용하여 DOM 상태 안정성 보장
    nextTick(() => {
      scrollToBottom()
    })
  }
...
const handleFloatButtonClick = async () => {
  try {
    // 메시지 수가 60개를 초과할 때만 재설정 및 새로 고침
...
const handleScroll = (event: Event) => {
  ...
  // 자동 스크롤 보호 기간인 경우 강제로 바닥으로 간주
  if (isAutoScrolling.value) {
    isAtBottom.value = true
  } else {
    // 바닥 여부 상태 업데이트
    const { scrollHeight, clientHeight } = container
    // 판단 임계값을 150px로 증가시켜 내결함성 향상
...
const debouncedScrollOperations = useDebounceFn(async (container: HTMLElement) => {
  ...
  if (scrollTop.value < 60) {
    // 로드 중이거나 이미 로드가 트리거되었거나 마지막 페이지에 도달한 경우 중복 트리거 안 함
...
// 채팅 영역 클릭 이벤트 처리, 답장 스타일 및 말풍선 활성 상태 지우기용
const handleChatAreaClick = (event: Event): void => {
  ...
  // 답장 관련 요소가 아닌 경우 activeReply 스타일 지우기
...
const handleLoadMore = async (): Promise<void> => {
  // 로드 중, 이미 로드 트리거됨, 또는 마지막 페이지 도달 시 중복 트리거 안 함
  ...
  // 로드 전 스크롤 높이 기록, 로드 후 위치 복원용
  const oldScrollHeight = container.scrollHeight
  const oldScrollTop = container.scrollTop
  try {
    await chatStore.loadMore()

    // 새 스크롤 위치 계산, 로드 전 사용자의 상대 위치 유지
    ...
    window.$message?.error('기록 메시지 로드 실패, 나중에 다시 시도해 주세요')
  } finally {
    isLoadingMore.value = false
...
.scrollbar-container {
  ...
  &.hide-scrollbar {
    ...
    // Mac에서 표시되지 않는 것을 방지하기 위해 작은 패딩 추가
...
// 드래그 시 마우스 이벤트 비활성화, 불필요한 리스너 손실 방지
