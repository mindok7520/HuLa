  ...
  <div ref="container" class="assistant-view">
    <div v-if="props.active && !isReady" class="assistant-view__placeholder">
      ...
      <span class="assistant-view__placeholder-text">
        {{ loadingProgress === 0 ? '모델 로드 시작...' : `모델 로드 중 ${loadingProgress}%` }}
      </span>
    </div>
  </div>
</template>
...
const resolveDracoDecoderPath = async () => {
  ...
  if (isDesktop()) {
    try {
      ...
      console.warn('Draco 디코더 리소스 경로 가져오기 실패, CDN으로 대체', error)
    }
  }
  ...
  if (!dracoLoader) {
    ...
    try {
      ...
      if (decoderPath !== DRACO_DECODER_BASE_URL) {
        console.warn('로컬 Draco 디코더 미리 로드 실패, CDN으로 대체', error)
        dracoDecoderBasePath = DRACO_DECODER_BASE_URL
    ...
    if (presetMeta) {
      ...
      if (isRemoteSource(props.customModel) && isDesktop()) {
        try {
          ...
          console.warn('원격 모델 캐싱 실패, 온라인 로드로 대체', error)
        }
      }
    }
...
const loadModel = async () => {
  if (!scene) {
    throw new Error('장면이 아직 초기화되지 않음')
  }
  loadingProgress.value = 0
  isReady.value = false
  if (!modelWrapper) {
  ...
  if (
    ...
    window.$message?.warning('압축된 glb 모델은 아직 지원되지 않습니다. 원본 모델 파일을 선택해 주세요')
  ...
  if (hasInvalidBounds) {
    window.$message?.warning('모델에 형상 데이터가 없거나 손상되었습니다. 확인 후 다시 가져오세요')
