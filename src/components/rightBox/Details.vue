...
const loadAnnouncement = async (roomId: string) => {
  ...
  try {
    ...
    console.error('그룹 공지사항을 가져오지 못했습니다:', error)
    announcementContent.value = ''
  }
}
...
watchEffect(async () => {
  if (content.type === RoomTypeEnum.SINGLE) {
    ...
    await getGroupDetail(content.uid)
      ...
      .catch((e) => {
        console.error('그룹 상세 정보를 가져오지 못했습니다:', e)
        announcementContent.value = ''
      })
  }
})
...
const fetchGroupMembers = async (roomId: string) => {
  try {
    ...
    console.error('그룹 멤버를 가져오지 못했습니다:', error)
  }
}
...
const openImageViewer = async () => {
  try {
    ...
    console.error('이미지 뷰어를 열지 못했습니다:', error)
  }
}
</script>
