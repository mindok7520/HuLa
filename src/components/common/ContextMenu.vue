...
/** 마우스 위치 및 우클릭 메뉴 표시 여부 가져오기 */
const { x, y, showMenu } = useContextMenu(ContextMenuRef, isNull)

// showMenu 상태 변경 감지 및 부모 컴포넌트로 이벤트 전송
watch(
  () => showMenu.value,
  (newVal) => {
...
/** 이모티콘 메뉴 크기 및 위치 */
const emojiWidth = ref(180) // 이모티콘 메뉴의 대략적인 너비, .emoji-container의 max-w-180px에 따라 설정

// 전체 이모티콘 표시 여부에 따라 메뉴 높이 동적 계산
const emojiHeight = computed(() => {
  return showAllEmojis.value ? 114 : 40 // 더보기 미표시 시 56, 펼침 시 126
})

/** 이모티콘 메뉴 위치 계산 */
const emojiMenuPosition = computed(() => {
  // 일반 우클릭 메뉴 계산 후 위치를 기준으로 사용
  let posX = pos.value.posX
  let posY = pos.value.posY - emojiHeight.value // 기본적으로 우클릭 메뉴 위에 표시

  // 메시지가 왼쪽인지 오른쪽인지 확인 (원본 마우스 위치와 화면 중심 관계를 통해)
  const isRightSideMessage = x.value > vw.value / 2

  if (isRightSideMessage) {
    // 이모티콘 메뉴 왼쪽 위치 = 우클릭 메뉴 오른쪽 경계 - 이모티콘 메뉴 너비
  ...
  // 수직 방향으로 뷰포트를 벗어나는지 확인
  if (posY < 10) {
    // 위쪽 공간이 부족하면 우클릭 메뉴 아래에 표시
...
// 메인 메뉴 표시 상태를 감지하는 watch 추가
watch(
  () => showMenu.value,
  (newVal) => {
    if (!newVal) {
      // 메인 메뉴 숨김 시 하위 메뉴도 숨김
...
const handleMouseEnter = (item: any, index: number) => {
  // 하위 메뉴가 있는지 확인 (함수 형태의 children 포함)
  ...
  if (rect.right + submenuWidth > vw.value) {
    // 오른쪽 공간 부족 시 아래쪽에 표시
    left = rect.left
    top = rect.bottom + 5 // 약간의 간격 추가

    // 아래쪽 공간 확인, 부족하면 위쪽에 표시
...
const handleMouseLeave = (e: MouseEvent) => {
  ...
  // 마우스가 하위 메뉴나 그 자식 요소로 이동하면 메뉴 닫지 않음
  if (relatedTarget?.closest('.context-submenu')) {
    return
  }

  // 메인 메뉴나 하위 메뉴 내부가 아니면 하위 메뉴 닫기
  setTimeout(() => {
