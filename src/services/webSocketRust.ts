...
class ListenerController {
  ...
  async abort(): Promise<void> {
    ...
    // 모든 정리 완료 대기 (차단 방지를 위해 시간 초과 설정)
    try {
      await Promise.race([
...
class RustWebSocketClient {
  ...
  //     info(`[RustWS] 효율적으로 ${oldListenerCount}개의 비즈니스 리스너 정리됨`)
  ...
  /**
   * 비즈니스 메시지 리스너 설정
   * Rust 측에서 전송한 특정 비즈니스 메시지 이벤트 수신
   */
  public async setupBusinessMessageListeners(): Promise<void> {
    ...
    this.listenerController.add(
      await listen('ws-room-info-change', (event: any) => {
        info(`방장이 그룹 채팅 정보 수정: ${JSON.stringify(event.payload)}`)
    ...
    this.listenerController.add(
      await listen('ws-my-room-info-change', (event: any) => {
        info(`본인이 그룹 내 내 정보 수정: ${JSON.stringify(event.payload)}`)
    ...
    this.listenerController.add(
      await listen('ws-webrtc-signal', (event: any) => {
        info(`신호 메시지 수신: ${JSON.stringify(event.payload)}`)
...
// Tauri 기본 이벤트를 사용하여 창 포커스 변경 감지 (크로스 플랫폼 호환)
// 창 포커스 리스너 중복 설정 방지
...
//       info('[RustWS] 창 포커스 이벤트 리스너 설정 완료')
//     } catch (err) {
