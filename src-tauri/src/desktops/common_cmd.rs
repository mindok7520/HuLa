...
pub fn screenshot(x: &str, y: &str, width: &str, height: &str) -> Result<String, String> {
    let screen = Screen::from_point(100, 100).map_err(|e| format!("화면 정보를 가져오는 데 실패했습니다: {}", e))?;
...
fn play_audio_internal(path: &str, handle: &AppHandle) -> Result<(), String> {
    ...
    let audio_path = handle
        ...
        .map_err(|e| format!("오디오 경로 해석 실패: {}", e))?;
...
pub fn set_height(height: u32, handle: AppHandle) -> Result<(), String> {
    let home_window = handle
        ...
        .ok_or("home 창을 찾을 수 없습니다")?;

    let sf = home_window
        .scale_factor()
        .map_err(|e| format!("창 배율 인자를 가져오는 데 실패했습니다: {}", e))?;

    let out_size = home_window
        .inner_size()
        .map_err(|e| format!("창 크기를 가져오는 데 실패했습니다: {}", e))?;
...
/// Mac 창의 제목 표시줄 버튼(신호등 버튼) 및 제목 숨기기
///
/// # 매개변수
/// * `window_label` - 창의 레이블 이름
/// * `hide_close_button` - 선택적 매개변수, 닫기 버튼 숨기기 여부, 기본값은 false(숨기지 않음)
/// * `handle` - Tauri 앱 핸들
///
/// # 반환
...
/// macOS 창 레벨을 화면 보호기 레벨로 설정하여 메뉴 바를 덮음
#[tauri::command]
#[cfg(target_os = "macos")]
pub fn set_window_level_above_menubar(window_label: &str, handle: AppHandle) -> Result<(), String> {
    ...
    // 창 레벨을 화면 보호기 레벨(1000)로 설정, 메뉴 바보다 높음
    ...
    for (i, &subkey) in registry_paths.iter().enumerate() {
        ...
        if result.is_ok() {
            ...
            if result.is_ok() && value_type == REG_DWORD && data > 0 {
                if i == 2 {
                    // LogPixels는 DPI 값이며 특수 처리가 필요함
                    return Ok(1.0); // LogPixels는 텍스트 배율이 아니며 기본값을 반환함
                } else {
                    // TextScaleFactor는 백분율 값임 (100 = 100%, 150 = 150%)
