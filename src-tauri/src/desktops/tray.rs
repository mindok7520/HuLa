...
pub fn create_tray<R: Runtime>(app: &tauri::AppHandle<R>) -> tauri::Result<()> {
    #[cfg(target_os = "macos")]
    {
        ...
        let open_menu_item =
            MenuItem::with_id(app, open_id.clone(), "주 패널 열기", true, None::<&str>)?;
        ...
        let _ = TrayIconBuilder::with_id("tray")
            .tooltip("HuLa")
            .icon(default_icon)
            .menu(&tray_menu) // 메뉴를 직접 설정하여 시스템이 우클릭 표시를 처리하도록 함
            .show_menu_on_left_click(false) // 좌클릭 시 메뉴 표시 비활성화
