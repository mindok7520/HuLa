    ...
    fn init_window_event(self) -> Self {
        self.on_window_event(|window, event: &WindowEvent| match event {
            WindowEvent::Focused(flag) => {
                // 사용자 정의 시스템 트레이 - 트레이 메뉴가 포커스를 잃을 때 숨김 구현
            ...
            WindowEvent::CloseRequested { .. } => {
                ...
                // 유효하지 않은 창인지 확인 (중요하지 않고 종료 가능한 창)
                ...
                // login 창이 사용자에 의해 닫힌 경우, 프로그램 즉시 종료
                else if win_label.eq("login") {
                    // 다른 창이 존재하는지 확인, home 창이 있으면 로그인 성공 후 정상 종료임
                    ...
                    // home 창이 있으면 로그인 성공 후 정상 종료이므로 닫기 허용
                }
            }
