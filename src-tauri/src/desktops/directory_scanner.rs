...
/// 진행률 이벤트가 포함된 디렉토리 크기 스캔
async fn get_directory_size_with_progress(
    directory_path: String,
    handle: AppHandle,
) -> Result<u64, String> {
    // 너무 큰 디렉토리를 스캔할 때 무기한 실행되는 것을 방지하기 위해 전체 타임아웃 메커니즘을 추가합니다.
    ...
    .await
    {
        ...
        Err(_) => {
            ...
            Err("디렉토리 스캔 시간이 초과되었습니다. 더 작은 디렉토리를 스캔해 보세요.".to_string())
        }
    }
}
...
async fn scan_directory_internal(directory_path: String, handle: AppHandle) -> Result<u64, String> {
    ...
    // 1. 총 파일 수를 계산하기 위해 빠른 사전 스캔을 수행합니다
    ...
    let _ = handle.emit(
        ...
        &DirectoryScanProgress {
            current_path: "파일 수를 계산 중...".to_string(),
            files_processed: 0,
            total_size: 0,
            elapsed_time: 0,
            elapsed_seconds: 0.0,
    ...
    loop {
        tokio::select! {
            ...
            _ = cancel_receiver.recv() => {
                ...
                return Err("스캔이 취소되었습니다".to_string());
            }
            // 파일 스캔 처리
            entry = entries.next() => {
                match entry {
                        ...
                        if entry.file_type().await.map_or(false, |ft| ft.is_file()) {
                            match entry.metadata().await {
                                Ok(metadata) => {
                                    ...
                                    // 진행률 업데이트: 200ms마나 또는 100개 파일마다 한 번씩 전송
                                    let now = Instant::now();
...
pub fn cancel_directory_scan() -> Result<(), String> {
    // 취소 신호를 보내 진행 중인 모든 스캔을 즉시 중단합니다
    let _ = CANCEL_SENDER.send(());
    Ok(())
}

/// 진행률 이벤트가 포함된 디렉토리 사용 정보
#[tauri::command]
pub async fn get_directory_usage_info_with_progress(
    ...
    for disk in &disks {
        ...
        // Windows에서는 경로 비교 시 경로 구분 기호를 처리해야 합니다
    ...
    if best_match.is_empty() {
        return Err("디렉토리가 위치한 디스크를 찾을 수 없습니다".to_string());
    }
