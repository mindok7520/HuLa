...
impl WebSocketClient {
        ...
        match current_state {
            ConnectionState::Connected => {
                ...
                if let Some(sender) = sender.as_ref() {
                    ...
                    // 메시지가 즉시 전송되지 않았음을 상위 계층에 알리기 위해 에러 반환
                    Err(anyhow::anyhow!(
            ...
            ConnectionState::Connecting | ConnectionState::Reconnecting => {
                ...
                pending.push(data);
                warn!(
                    "연결 중입니다. 메시지가 대기 큐에 추가되었습니다 (큐 길이: {})",
                    pending.len()
                );
            ...
            _ => {
                warn!("WebSocket이 연결되지 않았습니다 (상태: {:?}), 메시지를 전송할 수 없습니다", current_state);
                Err(anyhow::anyhow!(
    ...
    pub async fn get_health_status(&self) -> ConnectionHealth {
        ...
        let is_healthy = if last_pong == 0 {
            // 아직 pong을 받지 못한 경우 연결 상태에 따라 판단
            matches!(*self.state.read().await, ConnectionState::Connected)
        } else {
            now - last_pong < 30000 // 30초 이내에 pong을 받은 경우 헬시한 것으로 간주
        };
    ...
    pub async fn force_reconnect(&self) -> Result<()> {
        ...
        // 프론트엔드에 동기화 힌트를 표시하기 위해 재연결로 마킹
    ...
    async fn connection_loop(&self) -> Result<()> {
        loop {
            ...
            match self.try_connect().await {
                ...
                Err(e) => {
                    // max_reconnect_attempts가 0이면 무한 재연결을 의미하며, 오버플로 방지를 위해 포화 더하기 사용
                    let attempts = self
                        .reconnect_attempts
                    ...
                    // max_reconnect_attempts가 0이면 횟수 제한을 두지 않고, 그렇지 않으면 상한을 따름
                    if config.max_reconnect_attempts > 0
                    ...
                    // 지수 백오프 지연
                    // 백오프 단계를 상한으로 두어 attempts 무한 증가로 인한 거듭제곱 계산 오버플로 방지
    ...
    async fn process_business_message(message: &serde_json::Value, app_handle: &AppHandle) {
        ...
        match message_type {
            ...
            // 메시지 관련 TODO 현재 채팅 메시지 ACK만 구현됨
            "receiveMessage" => {
    ...
    async fn send_pending_messages(&self) -> Result<()> {
        ...
        if let Some(sender) = sender.as_ref() {
            ...
            // 실패한 메시지가 있으면 큐에 다시 추가
            if !failed_messages.is_empty() {
                let mut pending = self.pending_messages.write().await;
                for msg in failed_messages.into_iter().rev() {
                    pending.insert(0, msg); // 큐 앞부분에 삽입
                }
