...
pub fn initialize_keyboard_adjustment(webview_window: &WebviewWindow) {
    let _ = webview_window.with_webview(|webview| unsafe {
        ...
        // 키보드가 표시될 예정임을 감지하여 모드에 따라 레이아웃과 스크롤 조정.
        create_observer(
            &notification_center,
        ...
        let handling_mode_arc_observer = handling_mode_arc.clone();
        let webview_arc_observer = webview_arc.clone();
        // 키보드가 사라질 예정임을 감지하여 초기 레이아웃 복원 또는 편집 종료.
        create_observer(
            &notification_center,
            ...
            move |notification| {
                ...
                if current_mode == KeyboardHandlingMode::Adjust {
                    ...
                    *mode = KeyboardHandlingMode::None;
                    ...
                    // 잠금 모드에서는 스크롤 뷰의 기본 동작을 복원하고 키보드를 숨겨진 상태로 유지
                    let webview_ptr =
                    ...
                    // 원본 UIScrollViewDelegate를 복원하여 키보드 로직 종료 후 동작을 기본값으로 되돌림.
        ...
        let original_inset_arc_observer = original_inset_arc.clone();
        let handling_mode_arc_observer = handling_mode_arc.clone();
        // 키보드가 이미 표시되었음을 감지하여 최종 frame/inset을 정확히 동기화.
        create_observer(
            &notification_center,
            ...
            move |notification| {
                ...
                *keyboard_height = new_keyboard_height;
                ...
                // 키보드 표시 완료 후 원본 delegate를 복원하여 누적된 사용자 정의 delegate가 후속 스크롤 이벤트에 영향을 주지 않도록 함.
                if let Some(delegate) = old_delegate.take() {
                    scroll_view_arc_observer.setDelegate(Some(delegate.as_ref()));
                } else {
                    scroll_view_arc_observer.setDelegate(None);
...
/// 키보드 조정 모드를 전환하여 후속 알림 콜백이 해당 값에 따라 전략을 결정하도록 합니다.
...
define_class! {
    ...
    unsafe impl UIScrollViewDelegate for KeyboardScrollPreventDelegate {
        ...
        // 스크롤 콜백에서 강제로 초기 오프셋을 복원하여 사용자 드래그로 인한 어긋남 방지.
...
impl KeyboardScrollPreventDelegate {
    /// 스크롤 방지 delegate를 구축하고 대상 스크롤 뷰와 초기 오프셋을 저장합니다.
    fn new(
...
define_class! {
    ...
    unsafe impl UIScrollViewDelegate for KeyboardLockDelegate {
        ...
        // 사용자가 드래그를 시도하면 즉시 편집을 종료하고 고정 오프셋으로 복원.
...
impl KeyboardLockDelegate {
    /// 스크롤 잠금 delegate를 구축하고 스크롤 뷰, webview 및 고정 오프셋을 기록합니다.
    fn new(
        mtm: MainThreadMarker,
        scroll_view: Arc<Retained<UIScrollView>>,
...
/// 알림 리스너 등록을 캡슐화하고 클로저를 사용하여 받은 시스템 알림을 처리합니다.
fn create_observer(
