...
impl ImRequestClient {
    ...
    /// 요청을 빌드하는 공통 메서드 (요청을 보내지 않음)
    ///
    /// URL 빌드, 토큰 추가, body/params 처리 등 공통 로직 추출
    ///
    /// # 매개변수
    /// - `method`: HTTP 메서드
    /// - `path`: API 경로
            ...
            match result.code {
                Some(406) => {
                    if retry_count >= MAX_RETRY_COUNT {
                        return Err(anyhow::anyhow!("토큰 만료, 토큰 갱신 실패"));
                    }
                ...
                Some(401) => {
                    ...
                    return Err(anyhow::anyhow!("다시 로그인해주세요"));
                }
                Some(200) => {
    ...
    /// 스트리밍 요청 메서드 (SSE 등 스트리밍 응답용)
    ///
    /// `request` 메서드와의 차이점:
    /// 1. `Accept: text/event-stream` 요청 헤더 추가
    /// 2. JSON 파싱 대신 `reqwest::Response` 반환
    /// 3. 자동 토큰 갱신 재시도 미지원 (스트리밍 응답은 재시도를 중단할 수 없으므로)
    ///
    /// # 매개변수
    /// - `method`: HTTP 메서드
    /// - `path`: API 경로
    ...
    ///
    /// # 반환
    /// - `Ok(Response)`: 성공 시 응답 객체 반환, 스트리밍 데이터 읽기에 사용 가능
    /// - `Err`: 요청 실패 또는 상태 코드 2xx 아님
        ...
        if !status.is_success() {
            ...
            match status.as_u16() {
                406 => {
                    ...
                    return Err(anyhow::anyhow!("토큰 만료, 갱신 후 다시 시도해주세요"));
                }
                401 => {
    ...
    pub async fn start_refresh_token(&mut self) -> Result<(), anyhow::Error> {
        ...
        if !result.success {
            error!("토큰 갱신 실패: {}", result.msg.clone().unwrap_or_default());
            return Err(anyhow::anyhow!(
                "토큰 갱신 실패: {}",
                result.msg.clone().unwrap_or_default()
            ));
