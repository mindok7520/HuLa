...
fn process_image_url(url: &str, base_dir: &Path) -> String {
    // 절대 URL이거나 이미 data URI인 경우 직접 반환
...
pub async fn parse_markdown(app: AppHandle, file_path: String) -> Result<String, String> {
    ...
    let full_path = if file_path.starts_with('/') || file_path.starts_with("http") {
        ...
        // 개발 모드에서는 프로젝트 루트 디렉토리에서 읽음
        // 프로덕션 모드에서는 리소스 디렉토리에서 읽음
        ...
        // 프로덕션 모드 - 애플리케이션 리소스 디렉토리 (데스크톱 패키징 docs만 해당)
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        if let Ok(resource_dir) = app.path().resource_dir() {
            possible_paths.push(resource_dir.join(&file_path));
        }

        // 프로덕션 모드 - 애플리케이션 구성 디렉토리 (이전 버전 설치 경로 호환)
    ...
    // markdown 파일이 있는 디렉토리 가져오기, 상대 경로 파싱에 사용
...
pub async fn get_readme_html(app: AppHandle, language: String) -> Result<String, String> {
    // README 파일 경로 구축
    // 개발 모드에서는 프로젝트 루트 디렉토리에서 README.md 읽기
    // 프로덕션 모드에서는 리소스 디렉토리의 docs 폴더에서 README.md 읽기
    ...
    // README 파일이 있는 디렉토리 가져오기, 상대 경로 파싱에 사용
