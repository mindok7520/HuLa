...
const WRITE_RETRY_DELAY_MS: u64 = 80; // 재시도 기본 지연 시간 80ms

async fn run_with_write_lock<T, F, Fut>(
    lock: Arc<Mutex<()>>, // 전역 쓰기 잠금을 전달하여 순차 실행 보장
    op_name: &str,        // 로그용 현재 작업 이름
    mut operation: F,     // 실제 쓰기 로직
) -> Result<T, String>
where
    F: FnMut() -> Fut,                            // 비동기 쓰기 Future를 반환하는 클로저
    Fut: Future<Output = Result<T, CommonError>>, // 쓰기 결과 유형
{
    let mut attempt: usize = 0; // 현재 재시도 횟수
    loop {
        let guard = lock.lock().await; // 쓰기 잠금 획득
        let result = operation().await; // 실제 쓰기 실행
        drop(guard); // 쓰기 잠금 해제

        match result {
            Ok(val) => return Ok(val), // 성공 시 즉시 반환
            Err(err) => {
                let err_msg = err.to_string(); // 오류 메시지 기록
                let lowered = err_msg.to_lowercase(); // 일치 여부 확인을 위해 대소문자 통일
                    lowered.contains("database is locked") || lowered.contains("database is busy"); // 잠금 충돌 여부 감지

                if is_locked && attempt + 1 < WRITE_RETRY_LIMIT {
                    let delay = WRITE_RETRY_DELAY_MS * (attempt as u64 + 1); // 지연 시간 증가
                    warn!(
                        target: "tauri_db",
...
pub fn convert_message_to_resp(
    ...
    // 메시지 마크 파싱 - message_marks 필드에서 파싱 지원
...
pub async fn check_user_init_and_fetch_messages(
    ...
    // 빈번한 동기화 방지, 10초 이내에 한 번만 동기화 허용 (예: 불안정한 네트워크 상황에서 중복 재연결 방지)
        ...
        .await
    {
        if let Some(user_model) = user {
            ...
            // is_init가 true이면 백엔드 인터페이스를 호출하여 모든 메시지를 가져오고, 그렇지 않으면 증분 모드로 동기화
            if should_full_sync {
                info!(
                ...
                if let Err(e) = fetch_all_messages(client, db_conn, uid, async_data).await {
                    ...
                    "사용자 {} 증분/오프라인 메시지 업데이트, async_data: {:?}",
                    uid, async_data
                );
...
pub async fn fetch_all_messages(
    ...
    if let Some(mut messages) = messages {
        ...
        // time_block 일괄 계산 (먼저 계산 후 일괄 저장)
        // DB의 마지막 메시지 전송 시간을 기점으로 배치 내 항목별로 진행
        ...
        for (_index, msg_resp) in messages.iter_mut().enumerate() {
            ...
            // 해당 방의 이전 전송 시간 가져오기 (배치 내 최신값 우선, 없으면 DB에서 조회)
        ...
        // 메시지 저장이 완료되면 사용자의 is_init 상태를 false로 설정
...
pub async fn send_msg(
    ...
    let (login_uid, nickname) = {
        let user_info = state.user_info.lock().await;
        (user_info.uid.clone(), None) // UserInfo에는 uid와 token 필드만 있으므로 nickname은 임시로 None으로 설정
    };

    // 메시지 ID 생성
    let current_time = chrono::Utc::now().timestamp_millis();

    // 소유권 문제를 피하기 위해 먼저 data 클론
    ...
    let write_lock = state.write_lock.clone(); // 전역 쓰기 잠금 핸들 클론
    message_record = run_with_write_lock(write_lock, "send_msg", || {
        let db_conn = state.db_conn.clone(); // 비동기 사용을 위해 데이터베이스 연결 클론
        let mut record = message_record.clone(); // 클로저 내에서 가변성을 위해 메시지 기록 복사
        async move {
...
pub async fn delete_message(
    ...
    let resolved_room_id = if let Some(room) = room_id {
        ...
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "메시지가 존재하지 않거나 방 정보가 누락되었습니다".to_string())?
    };
