...
pub async fn get_config_by_key(
    ...
    let config = im_config::Entity::find()
        ...
        .one(db)
        .await
        .map_err(|e| anyhow::anyhow!("설정 조회 실패: {}", e))?;
    Ok(config)
}
...
pub async fn save_config_batch(
    ...
    // 현재 사용자의 기존 설정 데이터를 먼저 삭제
    im_config::Entity::delete_many()
        .filter(im_config::Column::LoginUid.eq(login_uid))
        .exec(&txn)
        .await
        .map_err(|e| anyhow::anyhow!("기존 설정 데이터 삭제 실패: {}", e))?;
    ...
    if !active_models.is_empty() {
        im_config::Entity::insert_many(active_models)
            .exec(&txn)
            .await
            .map_err(|e| anyhow::anyhow!("설정 데이터 일괄 삽입 실패: {}", e))?;
    }
...
pub async fn delete_config(
    ...
    im_config::Entity::delete_many()
        ...
        .exec(db)
        .await
        .map_err(|e| anyhow::anyhow!("설정 삭제 실패: {}", e))?;
    Ok(())
}
