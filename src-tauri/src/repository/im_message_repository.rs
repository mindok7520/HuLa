...
where
    C: ConnectionTrait,
{
    // 일괄 데이터베이스 작업을 위한 타임아웃 메커니즘 추가
    let timeout_duration = tokio::time::Duration::from_secs(120); // 2분 타임아웃
...
where
    C: ConnectionTrait,
{
    // SQLite의 변수 제한은 일반적으로 999이며, 안전을 위해 배치 크기를 50으로 설정합니다.
    // 먼저 조회하고 삭제한 다음 삽입해야 하므로 변수 제한을 피하기 위해 배치 크기를 줄입니다.
    const BATCH_SIZE: usize = 50;

    // 데이터량이 배치 크기보다 작으면 직접 처리
...
/// 방 ID에 따른 커서 기반 메시지 페이지 조회 (메시지 마크 포함)
pub async fn cursor_page_messages(
    ...
    // 먼저 메시지 메인 테이블을 조회하고 id 값 기준 내림차순 정렬
    ...
    if !cursor_page_param.cursor.is_empty() {
        // 커서 값을 사용하여 필터링하고 해당 ID보다 작은 레코드를 가져옴 (내림차순 정렬이므로)
...
pub async fn save_message(
    ...
    // 누락된 경우 time_block을 채우고 통합 계산 함수를 사용함
...
/// 방의 첫 번째 메시지인 경우 항상 시간을 표시하도록 Some(1)을 반환
    ...
    C: ConnectionTrait,
{
    ...
    let last_message = im_message::Entity::find()
        ...
        .order_by_desc(im_message::Column::SendTime) // 전송 시간 기준 내림차순
        .limit(1)
        .one(db)
        .await
...
/// 메시지 유형 필터링, 키워드 검색, 날짜 정렬 및 페이지네이션 지원
pub async fn query_chat_history(
    ...
    let messages = query
        .all(db)
        .await
        .map_err(|e| anyhow::anyhow!("채팅 히스토리 조회 실패: {}", e))?;

    enrich_models_with_thumbnails(db, messages).await
}

/// 파일 관리 전용 조회 함수, 여러 방에 걸친 파일 유형 메시지 조회 지원
pub async fn query_file_messages(
    ...
    if let Some(keyword) = search_keyword {
        ...
        if !trimmed_keyword.is_empty() {
            ...
            // JSON에 필드가 없을 경우 body에서 직접 검색할 수 있도록 호환성 유지
            keyword_condition = keyword_condition.add(Expr::cust_with_values(
                "LOWER(body) LIKE ?",
                [Value::from(keyword_pattern.clone())],
            ));
